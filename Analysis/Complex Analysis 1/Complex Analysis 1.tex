...
\begin{defn}{Standard Metric on $\C$}{}\\
Define the standard metric on $\C$ to be the function $d:\C\times\C\to\R$ by the formula $$d(a+bi,c+di)=\sqrt{(a-c)^2+(b-d)^2}$$
\end{defn}

\begin{prp}{}{}\\
Let $f,g:\C\to\C$ be functions. Let $z_0\in\C$. Suppose that the limits $\lim_{z\to z_0}f(z)$ and $\lim_{z\to z_0}g(z)$ exists. Then the following are true. 
\begin{itemize}
...
\subsection{The Modulus and Argument of a Complex Number}
\begin{defn}{Modulus}{}\\
Let $a+bi\in\C$ be a complex number. Define the modulus of the number to be $$\abs{a+bi}=\sqrt{a^2+b^2}$$
\end{defn}
...
\begin{defn}{Exponential Function}{} The exponential function is defined to be the function $$e^z=\sum_{k=0}^\infty\frac{z^k}{k!}=\lim_{n\to\infty}\sum_{k=0}^n\frac{z^k}{k!}$$ for all $z\in\C$. 
\end{defn}

\begin{prp}{}{}\\
For all $z\in\C$, the limit $e^z$ is convergent. 
\end{prp}
...
\subsection{The Polar Form and Exponential Form of Complex Numbers}
...
\begin{thm}{The Exponential Form of a Complex Number}{} Every $z\in\C$ can be written in the form $$z=re^{i\theta}$$ where $r=\abs{z}>0$ and $\theta\in\arg(z)$. 
\end{thm}
...
Recall that the standard norm on $\C$ is defined by $$\abs{a+bi}=\sqrt{a^2+b^2}$$ for $a,b\in\R$. This induces a metric $d:\C\times\C\to\R$ on $\C$ defined by $$d(z_1-z_2)=\abs{z_1-z_2}$$ for $z_1,z_2\in\C$. From topology, we know that the metric then induces a topology on $\C$, making $\C$ a topological space whose basis is given by $\{B_r(z)\;|\;r\in\R^+,z\in\C\}$. 
...
\begin{thm}{Cauchy Riemann Equations}{} Let $U\subseteq\C$ be open. Let $f(x+iy)=u(x,y)+iv(x,y)$ be real differentiable on $U$. Then $f$ is holomorphic on $U$ if and only if $$\frac{\partial u(x,y)}{\partial x}=\frac{\partial v(x,y)}{\partial y}$$ and $$\frac{\partial v(x,y)}{\partial x}=-\frac{\partial u(x,y)}{\partial y}$$
\begin{proof}
...
\begin{lmm}{}{} Let $f:\Omega\subseteq\C\to\C$ be real differentiable. Then $f$ is complex differentiable if and only if the following alternate version of Cauchy Riemann Equations are satisfied: $$f_{\overline{z}}=0$$ In this case, the complex derivative at $z\in\Omega$ is precisely $$f'(z)=f_z$$ 
\begin{proof}
