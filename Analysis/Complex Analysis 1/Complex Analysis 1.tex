...
\pagebreak
\section{Metric Properties of $\C$}
\subsection{The Polar Form}
Recall that from real analysis, we defined the power series $$\cos(x)=\sum_{k=0}^\infty\frac{(-1)^k}{(2k)!}x^{2k}\;\;\text{ and }\;\;\sin(x)=\sum_{k=0}^\infty\frac{(-1)^k}{(2k+1)!}x^{2k+1}$$

\begin{defn}{Argument}{}\\
Define the argument function $\text{arg}:\C\to\mP(\R)$ by the formula $$\text{arg}(a+bi)=\left\{\theta\in\R\;\bigg{|}\;\cos(\theta)=\frac{a}{\abs{a+bi}}\text{ and }\sin(\theta)=\frac{b}{\abs{a+bi}}\right\}$$
\end{defn}

\begin{prp}{}{}\\
The following are true regarding the argument. 
\begin{itemize}
...
\begin{lmm}{}{}\\
Let $z\in\C$ be a complex number. Then there exists a unique real number $\theta\in\R$ such that $\theta\in\arg(z)$ and $-\pi<\theta\leq\pi$. 
\begin{proof}\\
This is true since $\cos$ and $\sin$ are $2\pi$-periodic and $-1\leq\frac{a}{\abs{a+bi}},\frac{b}{\abs{a+bi}}\leq 1$. 
\end{proof}
\end{lmm}

\begin{defn}{Principal Argument}{}\\
Define the principal argument function $\text{Arg}:\C\to\R$ to be the function sending $z\in\C$ to the unique real number $\text{Arg}(z)\in\R$ such that $-\pi<\text{Arg}(z)\leq\pi$ and $\text{Arg}(z)\in\arg(z)$. 
\end{defn}

\begin{lmm}{}{}\\
Let $z\in\C$ be a complex number. Then we have $$\arg(z)=\{\text{Arg}(z)+2\pi k\;|\;k\in\Z\}$$
\begin{proof}\\
Again this is true since $\cos$ and $\sin$ are $2\pi$-periodic. 
\end{proof}
\end{lmm}

\begin{prp}{}{}\\
Let $z\in\C$ be a complex number. Then we have $$z=\abs{z}\left(\cos(\text{Arg}(z))+i\sin(\text{Arg}(z))\right)$$
\begin{proof}\\
Let $z=a+bi$. Then we have $$\abs{z}\left(\cos(\text{Arg}(z))+i\sin(\text{Arg}(z))\right)=\abs{a+bi}\left(\frac{a}{\abs{a+bi}}+i\frac{b}{\abs{a+bi}}\right)=a+bi=z$$
\end{proof}
\end{prp}

\begin{defn}{Polar Form}{}\\
Let $z\in\C$ be a complex number. Define the polar form of $z$ to be the expression $$\abs{z}\left(\cos(\text{Arg}(z))+i\sin(\text{Arg}(z))\right)$$
\end{defn}
...
\subsection{Limits and Continuity in $\C$}
Note that the modulus function $\abs{-}:\C\to\R$ defines a norm on $\C$. From Point Set Topology we know that such a norm induces the metric $$d_\C(z,w)=\abs{z-w}$$ and hence a topology on $\C$. The topology is defined as one would expect. For instance, the topology on $\C$ has a basis of open balls $$B_r(z)=\{x\in\C\;|\;\abs{z-x}<r\}$$ for all $r>0$ and $z\in\C$. In particular, there is a homeomorphism $\C\cong\R^2$ given by sending $a+bi$ to the pair $(a,b)$. \\

Since we have a metric / topology on $\C$, it makes sense to talk about continuous functions and limits of functions from $\C$ to $\C$. 

\begin{prp}{}{}\\
Let $U\subseteq\C$ be open. Let $f,g:U\to\C$ be functions. Let $z_0\in U$. Suppose that the limits $$\lim_{z\to z_0}f(z)\;\;\;\text{ and }\;\;\;\lim_{z\to z_0}g(z)$$ exists. Then the following are true. 
\begin{itemize}
\item $\lim_{z\to z_0}(f+g)(z)=\lim_{z\to z_0}f(z)+\lim_{z\to z_0}g(z)$. 
\item $\lim_{z\to z_0}(fg)(z)=\lim_{z\to z_0}f(z)\cdot\lim_{z\to z_0}g(z)$. 
\item If $\lim_{z\to z_0}g(z)\neq 0$, then we have $$\lim_{z\to z_0}\left(\frac{f}{g}\right)(z)=\frac{\lim_{z\to z_0}f(z)}{\lim_{z\to z_0}g(z)}$$
\end{itemize}
\end{prp}

\begin{prp}{}{}\\
Let $U\subseteq\C$ be open. Let $f,g:U\to\C$ be continuous functions. Let $z\in\C$. Then the following are true. 
\begin{itemize}
\item $f+g$ is continuous at $z$. 
\item $fg$ is continuous at $z$. 
\item If $g(z)\neq 0$, then $f/g$ is continuous at $z$. 
\end{itemize}
\end{prp}
...
Let $U\subseteq\C$ be open. Let $f:U\to\C$ be a function. Let $z_0\in U$. We say that $f$ is complex differentiable at $z_0$ if $$\lim_{z\to z_0}\frac{f(z)-f(z_0)}{z-z_0}$$ exists. In this case we denote the limit as $f'(z_0)$. 
\end{defn}

Equivalently, we say that $f$ is differentiable at $z_0$ if the limit $$\lim_{h\to 0}\frac{f(z_0+h)-f(z_0)}{h}$$ exists where $h\in\C$. This can be seen by a simple substitution trick of the limit similar to that of the real version. 

\begin{eg}{}{}\\
Let $n\in\N$. Let $z_0\in\C$. Then the function $f:\C\to\C$ defined by $f(z)=z_0$ is differentiable. 
\begin{proof}\\
We compute the limit to be $$\lim_{h\to 0}\frac{z_0-z_0}{h}=0$$ using the epsilon delta definition of limits. 
\end{proof}
\end{eg}

\begin{eg}{}{}\\
Let $n\in\N$. Then the function $f:\C\to\C$ defined by $f(z)=z$ is differentiable. 
\begin{proof}\\
We compute the limit to be $$\lim_{h\to 0}\frac{z+h-z}{h}=1$$ using the epsilon delta definition of limits. 
\end{proof}
\end{eg}

Recall from Multivariable calculus that the limit of a real function in several variables may not exist even if the limits along the $x$ or $y$ axis exists. Because complex functions also have limits coming from different directions, we cannot check differentiability on components like we did with continuity. 

\begin{eg}{}{}\\
The function $f(z)=\overline{z}$ is not complex differentiable (but it is differentiable in the real sense). 
\begin{proof}\\
Notice that the limit simplifies to $$\lim_{h\to 0}\frac{\overline{z+h}-\overline{z}}{h}=\lim_{h\to 0}\frac{\overline{h}}{h}$$ If $h$ is real, then the limit is $1$. If $h$ is imaginary, then the limit is $0$. Thus the limit does not exists. 
\end{proof}
\end{eg}

\begin{prp}{}{}\\
Let $U\subseteq\C$ be open. Let $f:U\to\C$ be a function. Let $z\in\C$. If $f$ is differentiable at $z$, then $f$ is continuous at $z$. 
\end{prp}

\begin{prp}{}{}\\
Let $U\subseteq\C$ be open. Let $f,g:U\to\C$ be functions. Let $z\in\C$. If $f$ and $g$ are differentiable at $z$, then the following are true. 
\begin{itemize}
\item $f+g$ is differentiable at $z$, and $(f+g)'(z)=f'(z)+g'(z)$. 
\item $fg$ is differentiable at $z$, and $(fg)'(z)=f'(z)g(z)+f(z)g'(z)$. 
\item If $g'(z)\neq 0$, then $f/g$ is differentiable and $(f/g)'(z)=f'(z)/g'(z)$. 
\end{itemize}
\end{prp}

\begin{prp}{}{}\\
Let $U\subseteq\C$ be open. Let $f:U\to\C$ and $g:\im(f)\to\C$ be functions. Let $z\in\C$. If $f$ is differentiable at $z$ and $g$ is differentiable at $f(z)$, then $g\circ f$ is differentiable at $z$ and $$(g\circ f)'(z)=g'(f(z))f'(z)$$
\end{prp}

\begin{eg}{}{}\\
Let $f:\C\to\C$ be a polynomial. Then $f$ is complex differentiable. 
\begin{proof}\\
Indeed, we have seen that the identity function and constant functions are differentiable, and polynomials are the finite sums and products of these types of functions. 
\end{proof}
\end{eg}

\begin{defn}{Holomorphic, Biholomorphic and Meromorphic Functions}{}\\
Let $U,V\subseteq\C$ be open. Let $f:U\to V$ be a function. 
\begin{itemize}
\item We say that $f$ is holomorphic if $f$ is complex differentiable at all points in $U$. 
\item We say that $f$ is biholomorphic if $f$ is a bijection such that $f$ and $f^{-1}$ are holomorphic. 
\item We say that $f$ is meromorphic if $f$ is complex differentiable at all $x\in U\setminus\{w\in\C\;|\;f(w)=\infty\}$ and $1/f$ is complex differentiable at all $x\in U\setminus\{w\in\C\;|\;f(w)=0\}$. 
\end{itemize}
\end{defn}
...
Let $U\subseteq\C$ be open. Let $f(x+iy)=u(x,y)+iv(x,y)$ be real differentiable on $U$. Then $f$ is holomorphic on $U$ if and only if $$\frac{\partial u(x,y)}{\partial x}=\frac{\partial v(x,y)}{\partial y}\;\;\;\text{ and }\;\;\;\frac{\partial v(x,y)}{\partial x}=-\frac{\partial u(x,y)}{\partial y}$$
\begin{proof}
...
\begin{defn}{Partial Derivatives in z}{}\\
Let $U\subseteq\C$ be open. Let $f:U\to\C$ be real differentiable. Define $$f_z=\frac{\partial f}{\partial z}=\frac{1}{2}\left(\frac{\partial f}{\partial x}-i\frac{\partial f}{\partial y}\right)\;\;\;\;\text{ and }\;\;\;\;f_{\overline{z}}=\frac{\partial f}{\partial \overline{z}}=\frac{1}{2}\left(\frac{\partial f}{\partial x}+i\frac{\partial f}{\partial y}\right)$$
\end{defn}

Under these definition, the Cauchy Riemann equations becomes simple. 

\begin{lmm}{}{}\\
Let $U\subseteq\C$ be open. Let $f:U\to\C$ be real differentiable. Then $f$ is holomorphic on $U$ if and only if the following alternate version of Cauchy Riemann Equations are satisfied: $$f_{\overline{z}}(x)=0$$ for all $x\in U$. In this case, the complex derivative at $x\in U$ is precisely $$f'(x)=f_z(x)$$ 
\begin{proof}
...
\begin{lmm}{}{} Let $U_1,U_2$ be open. If $f:U_1\to U_2$ is a biholomorphic map then both $f$ and $f^{-1}$ are conformal. 
\begin{proof}
...
A power series in $\C$ is an infinite series of the form $$\sum_{k=0}^{\infty}a_k(z-z_0)^k$$ where $a_n\in\C$ for all $n\in\N$ and $z_0\in\C$ is a fixed complex number. 
\end{defn}

Similar to the real case, we only consider power series whose center $z_0=0$ is at $0$ because the general more setting is recovered by a translation. 

\begin{prp}{}{}\\
Let $\sum_{k=0}^\infty a_kz^k$ be a power series. Let $z_0\in\C$. If $\sum_{k=0}^\infty a_kz_0^k$ is convergent, then $\sum_{k=0}^\infty a_kw^k$ is convergent for all $\abs{w}<\abs{z_0}$. 
\end{prp}

\begin{defn}{Radius of Convergence}{}\\
Let $\sum_{k=0}^\infty a_kz^k$ be a power series Define the radius of convergence of the power series to be $$R=\sup\left\{z\in\C\;\bigg{|}\;\sum_{k=0}^\infty a_kz^k\text{ converges }\right\}$$
\end{defn}
...
\begin{prp}{}{}\\
Let $\sum_{k=0}^\infty a_kz^k$ be a power series. Then the radius of convergence of the power series is given by $$R=\frac{1}{\limsup_{n\to\infty}\abs{a_n}^{\frac{1}{n}}}$$
\end{prp}

\begin{prp}{}{}\\
Let $\sum_{k=0}^\infty a_kz^k$ be a power series. Let $R$ be the radius of convergence. Then the following are true. 
\begin{itemize}
\item The function $f:B_r(0)\to\C$ given by $f(z)=\sum_{k=0}^\infty a_kz^k$ is holomorphic. 
\item The derivative of $f$ is given by $$f'(z)=\sum_{k=1}^\infty ka_kz^{k-1}$$
\end{itemize}
\end{prp}

\subsection{The Complex Exponential and Logarithm}
\begin{lmm}{}{}\\
The radius of convergence of the power series $\sum_{k=0}^\infty\frac{z^k}{k!}$ is $\infty$. 
\end{lmm}

\begin{defn}{Exponential Function}{}\\
Let $z\in\C$. Define the complex exponential of $z$ to be $$e^z=\sum_{k=0}^\infty\frac{z^k}{k!}=\lim_{n\to\infty}\sum_{k=0}^n\frac{z^k}{k!}$$ for all $z\in\C$. 
\end{defn}

\begin{prp}{}{}\\
The following are true regarding the complex exponential. 
\begin{itemize}
\item $e^{z+w}=e^ze^w$ for all $z,w\in\C$. 
\item $(e^z)^w=e^{zw}$ for all $z,w\in\C$. 
...
Let $\theta\in\R$. Then we have $$e^{i\theta}=\cos(\theta)+i\sin(\theta)$$
\end{thm}
...
\begin{prp}{}{}\\
Let $z\in\C$. Then we have $$z=\abs{z}e^{i\text{Arg}(z)}$$
\end{prp}
...
\subsection{The Complex Trigonometric Functions}
\begin{lmm}{}{}\\
The radius of convergence of the two power series $$\sum_{k=0}^\infty\frac{(-1)^kz^{2k+1}}{(2k+1)!}\;\;\;\text{ and }\;\;\;\sum_{k=0}^\infty\frac{(-1)^kz^{2k}}{(2k)!}$$ is $\infty$. 
\end{lmm}

\begin{defn}{The Complex Sine and Cosine}{}\\
Define the sine function $\sin:\R\to\R$ to be the power series $$\sin(z)=\sum_{k=0}^\infty\frac{(-1)^kz^{2k+1}}{(2k+1)!}$$ Define the cosine function $\cos:\R\to\R$ to be the power series $$\cos(z)=\sum_{k=0}^\infty\frac{(-1)^kz^{2k}}{(2k)!}$$
\end{defn}

\begin{prp}{The Euler Formulas}{}\\
The following are true. 
\begin{itemize}
\item $\cos(z)=(e^{iz}+e^{-iz})/2$. 
\item $\sin(z)=(e^{iz}-e^{-iz})/(2i)$. 
\end{itemize}
\end{prp}

\subsection{Analytic Functions}
\begin{defn}{Analytic Functions}{}\\
Let $U\subseteq\C$ be open. Let $f:U\to\C$ be a function. We say that $f$ is analytic if for each $w\in U$, we can write $f$ as a power series $$f(z)=\sum_{k=0}^\infty a_k(z-w)^k$$ for all $z\in B_r(w)$ for some $r>0$. 
\end{defn}
...
\begin{prp}{}{} Let $f:\Omega\subseteq\C\to\C$ be continuous. Let $\gamma:[a,b]\to\Omega$. Let $\gamma^-$ represent $\gamma$ in the opposite direction. Then $$\int_{\gamma^-}f(z)\,dz=-\int_\gamma f(z)\,dz$$ 
...
\begin{prp}{Independence of Equivalent Parametrization}{} Suppose that $\gamma_1:[a,b]\to\Omega\subseteq\C$ and $\gamma_2:[c,d]\to\Omega$ are equivalent parametrizations. Let $f:\Omega\to\C$ be a continuous function. Then $$\int_{\gamma_1}f(z)\,dz=\int_{\gamma_2}f(z)\,dz$$ 
...
\begin{thm}{}{} Let $f:\Omega\subseteq\C\to\C$ be continuous. Suppose that $f$ has an antiderivative $F$. Let $a,b\in U$. Let $\gamma$ be a path joining $a$ to $b$. Then $$\int_{\gamma}f=F(\gamma(b))-F(\gamma(a))$$ 
...
\begin{lmm}{}{} Let $\gamma:[a,b]\to\C$ be a closed continuous path that avoids a radial line for some point $w\in\C$ to $\infty$, then $I(\gamma,w)=0$. 
\begin{proof}
...
\begin{thm}{}{} Let $\gamma$ be a closed continuous path. Then $$I(\gamma,0)=\frac{1}{2\pi i}\int_{\gamma}\frac{1}{z}\,dz$$ More specifically, $$I(\gamma,z_0)=\frac{1}{2\pi i}\int_{\gamma}\frac{1}{z-z_0}\,dz$$ 
\begin{proof}
...
\begin{thm}{Goursat's Theorem}{} Let $U\subseteq\C$ be open and let $R$ be a triangle contained in $U$. Let $f:U\to\C$ be holomorphic. Then $$\int_{\partial R}f=0$$ 
...
\begin{thm}{}{} Suppose that $\Omega$ is a star-shaped domain, and $f:\Omega\to\C$ is a continuous function. Suppose that for every closed triangle $T\subset\Omega$ we have $$\int_{\partial T}f(z)\,dz=0$$ Then there exists a holomorphic function $F:\Omega\to\C$ such that $F'(z)=f(z)$. Specifically, we can choose $$F(z)=\int_{[z_0,z]}f(w)\,dw$$ 
\begin{proof}
...
\begin{crl}{}{} Suppose that $\Omega$ is a star-shaped domain, and $f:\Omega\to\C$ is a holomorphic function. Then there exists a holomorphic function $F:\Omega\to\C$ such that $F'(z)=f(z)$. 
\begin{proof}
...
\begin{thm}{Cauchy's Theorem on Star-Shaped Domain}{} Let $\Omega\subseteq\C$ be star-shaped. Let $f:\Omega\to\C$ be holomorphic. Let $\gamma$ be closed in $\Omega$. Then $$\int_{\gamma}f(z)\,dz=0$$ 
\begin{proof}
By the above theorem, $f$ has an antiderivative. We conclude by theorem 3.2.8. 
\end{proof}
\end{thm}

\begin{crl}{Cauchy's Theorem on Annuli}{} Suppose that $0\leq r_1<R_1<R_2<r_2$, and that $f$ is a holomorphic function on the annulus $A_{r_1,r_2}=\{z\in\C\;|\; r_1<\abs{z}<r_2\}$. Then writing $A=\{z\in\C\;|\; R_1<\abs{z}<R_2\}$, we have $$\int_{\partial A}f(z)\,dz=0$$ 
\begin{proof}
...
\begin{thm}{Cauchy's Integral Formula on a Disc}{} Let $U\subseteq\C$ be open. Let $\overline{B_r(a)}\subset U$. Let $f:U\to\C$ be holomorphic. Then for every $w\in B_r(a)$, we have $$f(w)=\frac{1}{2\pi i}\oint_{\partial B_r(a)}\frac{f(z)}{z-w}\,dz$$ 
\begin{proof}
...
\begin{thm}{Taylor's Theorem}{} Let $\Omega\subseteq\C$ be open. Let $f:\Omega\subseteq\C$ be holomorphic. Let $z_0\in\Omega$ and $r>0$ and $B_r(z_0)\subseteq\Omega$. Then all higher derivatives of $f$ exists and for all $z\in B_r(z_0)$ we can write $$f(z)=\sum_{k=0}^\infty a_k(z-z_0)^k$$ where $$a_k=\frac{1}{2\pi i}\int_{\partial B_r(z_0)}\frac{f(w)}{(w-z_0)^{k+1}}\,dw$$ 
...
\begin{crl}{}{} A function $f:\Omega\to\C$ is holomorphic on an open set $U$ if and only if it is analytic. 
...
\begin{thm}{Cauchy's Integral Formula}{} Let $U\subseteq\C$ be open. Let $f:U\to\C$ be holomorphic. Then $f$ is infinitely complex differentiable. In particular, if $\overline{B_r(z_0)}\subset U$, then we have the formula $$f^{(n)}(z_0)=\frac{n!}{2\pi i}\int_{\partial B_r(z_0)}\frac{f(w)}{(w-z_0)^{n+1}}\,dw$$ for each $n\in\N$. 
\begin{proof}
...
\begin{thm}{Mean Value Theorem}{} Let $U\subseteq\C$ be open and $\overline{B_r(z_0)}\subset U$ for some $r>0$. Let $z_0\in U$ and $f:U\to\C$ be holomorphic. Then $$f(z_0)=\frac{1}{2\pi}\int_0^{2\pi}f(z_0+re^{i\theta})\,d\theta$$ 
\begin{proof}
...
\begin{crl}{Cauchy's Estimate}{} Suppose that $f(z)=\sum_{k=0}^\infty a_kz^k$ is holomorphic on $B_r(0)$ for some $r>0$ and that for all $z\in B_r(0)$ we have $\abs{f(z)}\leq M$. Then $$\abs{a_k}\leq\frac{M}{r^k}$$ for all $k$. 
\begin{proof}
...
\begin{crl}{Liouville's Theorem}{} Any bounded holomorphic function is constant. 
\begin{proof}
...
\begin{thm}{Fundamental Theorem of Algebra}{} Every non-constant polynomial has at least one zero in $\C$. 
\begin{proof}
...
\begin{prp}{}{} Let $f:\C\to\C$ be a polynomial such that $f$ has degree $n$. Then $f$ has exactly $n$ roots in $\C$ counting repeated roots. 
\begin{proof}
...
\begin{thm}{Morera's Theorem}{} Suppose $U\subseteq\C$ is open and $f:U\to\C$ is continuous. Suppose that for all closed triangles $T\subset\Omega$ we have $$\int_{\partial T}f(z)\,dz=0$$ Then $f$ is holomorphic on $\Omega$. 
\begin{proof}
...
\begin{lmm}{Local Invertibility Lemma}{} Let $U\subseteq\C$ be open. Let $f:U\to\C$ is holomorphic with $f'(z_0)\neq 0$ at some $z_0\in U$. Then there exists a neighbourhood $V\subset U$ of $z_0$ and a neighbourhood $W\subset\C$ of $f(z_0)$ such that the restriction $f|_V:V\to W$ is biholomorphic. 
\begin{proof}
...
\begin{thm}{Cauchy's Theorem - Homology Version}{} Let $U\subseteq\C$ be open and $f:U\to\C$ holomorphic. Let $\gamma$ be a cycle that is homologous to $0$. Then we have $$\int_\gamma f(z)\,dz=0$$ 
\begin{proof}
...
In particular, each zero of finite order is an isolated point of the set of zeros. 
\begin{proof}
...
\begin{thm}{}{} Suppose that $\Omega\subset\C$ is open and connected. Let $f:\Omega\to\C$ be a holomorphic function that has a zero of infinite order at some $z_0\in\Omega$. Then $f$ is identically $0$. 
\begin{proof}
...
\begin{thm}{The Identity Theorem}{} Let $\Omega\subset\C$ be open and connected. Let $f,g:\Omega\to\C$ be holomorphic. If the set $$\{z\in\Omega|f(z)=g(z)\}$$ has at least one accumulation point in $\Omega$, then $f(z)=g(z)$ for all $z\in\Omega$. 
\begin{proof}
...
\begin{thm}{}{} Let $\Omega\subset\C$ be open. Let $f:\Omega\to\C$ be holomorphic with $z_0$ a zero of order $1\leq n<\infty$. Then there exists $z_0\in V\subset\Omega$ and a biholomorphic function $h:V\to B_r(0)$ for some $r>0$ such that $$f(z)=\left(h(z)\right)^k$$ for all $z\in V$. This means that $f$ is locally $k$-to-one near $z_0$. 
\begin{proof}
...
\begin{thm}{Open Mapping Theorem}{} Suppose $\Omega\subset\C$ is open and connected. Let $f:\Omega\to\C$ be holomorphic but not constant. Then the image $f(\Omega)$ is also open and connected. 
\begin{proof}
...
\begin{crl}{Maximum Modulus Principle}{} Let $\Omega\subset\C$ be open and connected. Let $f:\Omega\to\C$ be a holomorphic but not constant function. Then $\abs{f}$ does not have any local maxima. 
\begin{proof}
...
\begin{thm}{Riemann's Removable Singularity Theorem}{} Let $f:B_r(a)\setminus\{a\}\to\C$ be a holomorphic function. Suppose that $z_0$ is a removable singularity of $f$. Then $f$ can be extended to a holomorphic function $f:B_r(a)\to\C$. 
\begin{proof}
...
\begin{prp}{}{} Let $f:B_r(a)\setminus\{a\}\to\C$ be holomorphic function with $a$ a pole. Then there exists $n\in\N$ and a holomorphic function $g:B_r(a)\to\C$ such that $$f(z)=\frac{g(z)}{(z-a)^n}$$ and $g$ is non-zero in a neighbourhood of $a$. Moreover, $n$ is the order of $a$. 
\begin{proof}
...
\begin{thm}{Casorati-Weierstrass Theorem}{} Suppose that a holomorphic function $f:B_r(z_0)\setminus\{z_0\}\to\C$ has an essential singularity at $z_0$. Then for any $\delta\in(0,r)$, the image of the set $B_\delta(z_0)\setminus\{z_0\}$ under $f$ is dense in $\C$. 
\begin{proof}
...
\begin{thm}{Laurent's Theorem}{} Suppose $0\leq r_1<r_2$, $a\in\C$ and $f$ is holomorphic on $A=\{z\in\C|r_1<\abs{z-a}<r_2\}$ Then for every $z\in A$, we have $$f(z)=\sum_{k=-\infty}^\infty a_k(z-a)^k$$ where $$a_k=\frac{1}{2\pi i}\int_{\partial B_s(a)}\frac{f(w)}{(w-a)^{k+1}}\,dw$$ for all $s\in(r_1,r_2)$. 
\begin{proof}
...
\begin{thm}{Injective Entire Functions}{} Let $f:\C\to\C$ be injective and entire. Then $$f(z)=\alpha z+\beta$$ for some $\alpha\in\C\setminus\{0\}$ and $\beta\in\C$. 
\begin{proof}
...
\begin{thm}{Cauchy's Integral Formula - General Version}{} Let $U\subset\C$ be open and let $\gamma$ be a cycle in $U$ that is homologous to $0$. Then for any holomorphic function $f:U\to\C$ and for any $w\in\Omega$ not lying in the image of $\gamma$, we have $$f(w)I(\gamma,w)=\frac{1}{2\pi i}\int_\gamma\frac{f(z)}{z-w}\,dz$$ 
\begin{proof}
...
\begin{prp}{}{} Möbius Transformations are continuous and bijective. In particular, the inverse of a Möbius Transformation is also a Möbius Transformation given by the formula $$f^{-1}(z)=\frac{dz-b}{-cz+a}$$ 
\begin{proof}
...
\begin{lmm}{}{} Every Möbius Transformation can be written as the composition of elementary Möbius Transformations. 
\begin{proof}
...
\begin{lmm}{}{} Every Möbius Transformation $f:\C_\infty\to\C_\infty$ other than the identity has at least one but at most two fixed points. 
\begin{proof}
...
\begin{thm}{}{} Let $z_1,z_2,z_3\in\C_\infty$ be distinct. Also let $w_1,w_2,w_3\in\C_\infty$ be distinct, then there exists a unique Möbius Transformation $f$ such that $f(z_1)=w_1$, $f(z_2)=w_2$ and $f(z_3)=w_3$. 
\begin{proof}
...
\begin{thm}{}{} Let $w\in\C$ with $\abs{w}<1$. Then the Möbius transformation $$f(z)=\frac{z-w}{\overline{w}z-1}$$ is a bijective map from $D$ to $D$ that sends $\partial D$ to $\partial D$. 
\begin{proof}
...
\begin{prp}{Cayley Transformation}{} The upper half plane $$H=\{z=x+iy\in\C\;|\;y>0\}$$ is conformally equivalent to the unit disc by the biholomorphic function $f:\C\to\C$ defined by $$f(z)=\frac{z-i}{z+i}$$ 
\begin{proof}
We have that 
\begin{align*}
...
\begin{prp}{}{} The upper right quarter of the complex plane $$Q=\{z=x+iy\in\C\;|\;x>0,y>0\}$$ is conformally equivalent to the unit disc $D$. 
\begin{proof}
...
\begin{prp}{}{} The upper half disc $$D^+=\{z=x+iy\in\C\;|\;\abs{z}<1\text{ and }y>0\}$$ is conformally equivalent to the whole disc $D$. 
\begin{proof}
...
\begin{thm}{}{} Suppose $\Omega\subset\C$ is open and connected and $f:\Omega\to\C$ is both injective and holomorphic. Then $f(\Omega)$ is a open and connected set and $f$ is a biholomorphic map onto its image. 
\begin{proof}
...
\end{itemize}
If the first equality holds, or the second equality holds for some $z\in D$, then $f$ is a rotation in the sense that $f(z)=e^{i\theta}z$ for some $\theta\in\R$. 
\begin{proof}
...
\begin{thm}{}{} Every biholomorphic function $f:D\to D$ is a Mobius transformation of the form $$f(z)=e^{i\theta}\left(\frac{z-a}{\overline{a}z-1}\right)$$ for $\abs{a}<1$ and $\theta\in[-\pi,\pi)$. 
\begin{proof}
