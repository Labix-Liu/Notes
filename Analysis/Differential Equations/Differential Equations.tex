...
\section{}
\subsection{}
\begin{defn}{Partial Differential Equations}{}\\
Let $U\subseteq\R^n$ be an open subset. A partial differential equation on $U$ is an equation of the form $$F(x,u(x),Du(x),\dots,D^ku(x))=0$$ where $u:U\to\R$ is a $\mC^k$-function and $F:U\times\R\times\R^n\times\cdots\times\R^{n^k}$ is a function. 
\end{defn}

\begin{defn}{Linear Types of Partial Differential Equations}{}\\
Let $U\subseteq\R^n$ be an open subset. Let $u:U\to\R$ be a $\mC^k$-function. Let $F:U\times\R\times\R^n\times\cdots\times\R^{n^k}$ be a partial differential equation on $u$. 
\begin{itemize}
\item We say that $F$ is linear if $F$ is of the form $$F=\sum_{i=0}^kp_iD^iu-f$$ where $p_i:U\to\R$ for $0\leq i\leq k$ and $f$ are functions. 
\end{itemize}
\end{defn}

\begin{defn}{Homogeneous Partial Differential Equations}{}\\
Let $U\subseteq\R^n$ be an open subset. Let $u:U\to\R$ be a $\mC^k$-function. Let $F:U\times\R\times\R^n\times\cdots\times\R^{n^k}$ be a partial differential equation on $u$. We say that $F$ is homogeneous if 
\end{defn}

\pagebreak
\section{Four Important Linear Partial Differential Equations}
\subsection{Transport Equation}
\begin{defn}{Linear Transport Equation}{}\\
Define the transport equation to be the partial differential equation $$\frac{\partial u}{\partial t}+b\cdot\left(\frac{\partial u}{\partial x_1},\dots,\frac{\partial u}{\partial x_n}\right)=f$$ for some $b\in\R^n$ and some function $f:\R^n\times(0,\infty)\to\R$, and $u:\R^n\times(0,\infty)\to\R$ is the solution where we use the convention $u=u(x_1,\dots,x_n,t)$. 
\end{defn}

\begin{defn}{The Initial Value Problem for Linear Transport Equations}{}\\
The initial value problem for the linear transport equation consists of finding a function $u:\R^n\times(0,\infty)\to\R$ such that the following are true. 
\begin{itemize}
\item $u$ is the solution to the transport equation: $$\frac{\partial u(x,t)}{\partial t}+b\cdot\left(\frac{\partial u(x,t)}{\partial x_1},\dots,\frac{\partial u(x,t)}{\partial x_n}\right)=f(x,t)$$ for some $b\in\R^n$ and some function $f:\R^n\times(0,\infty)\to\R$. 
\item $u(-,0)=g$. 
\end{itemize}
\end{defn}

\begin{prp}{}{}\\
Let $b\in\R^n$. Let $f:\R^n\times(0,\infty)\to\R$ be an integrable function. Let $g:\R^n\to\R$ be a function. Then the function $u:\R^n\times(0,\infty)\to\R$ defined by $$u(x,t)=g(x-tb)+\int_0^tf(x+(s-t)b,s)\;ds$$ is a solution to the initial value problem for linear transport equations. 
\end{prp}
