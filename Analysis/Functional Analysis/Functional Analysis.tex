...
\pagebreak
\section{The Topology on Normed Spaces}
\subsection{Norms and Metrics}
\begin{defn}{Norms}{}\\
Let $\F$ be a field. Let $V$ be a vector space over $\F$. A norm on $V$ is a function $\|\cdot\|:V\to\R$ such that the following are true. 
\begin{itemize}
...
\end{itemize}
In this case we say that $(V,\|\cdot\|)$ is a normed vector space over $\R$. 
\end{defn}

\begin{thm}{The Parallelogram Law over $\R$}{}\\
Let $(V,\|\cdot\|)$ be a normed vector space over $\R$. Then there exists an inner product $\langle-,-\rangle$ such that $\|v\|=\sqrt{\langle v,v\rangle}$ for all $v\in V$ if and only if $$2\|x\|^2+2\|y\|^2=\|x+y\|^2+\|x-y\|^2$$ for all $x,y\in V$. 
\end{thm}

\begin{thm}{The Parallelogram Law over $\C$}{}\\
Let $(V,\|\cdot\|)$ be a normed vector space over $\C$. Then there exists a Hermitian inner product $\langle-,-\rangle$ such that $\|v\|=\sqrt{\langle v,v\rangle}$ for all $v\in V$ if and only if $$2\|x\|^2+2\|y\|^2=\|x+y\|^2+\|x-y\|^2$$ for all $x,y\in V$. 
\end{thm}

\begin{defn}{The Induced Metric from Norms}{}\\
Let $(V,\|\cdot\|)$ be a normed vector space over $\R$. Define the induced metric to be the function $d:V\times V\to\R$ given by $$d(x,y)=\|x-y\|$$
\end{defn}

\begin{defn}{Equivalent Norms}{}\\
Let $k$ be a field. Let $V$ be a vector space over $k$. Let $\|\cdot\|_1$ and $\|\cdot\|_2$ be two norms on $V$. We say that the two norms are equivalent if there exists $c_1,c_2\in k$ such that $$c_1\|x\|_1\leq\|x\|_2\leq c_2\|x\|_1$$ for all $x\in V$. 
\end{defn}

\begin{prp}{}{}\\
Let $k$ be a field. Let $V$ be a vector space over $k$. Let $\|\cdot\|_1$ and $\|\cdot\|_2$ be two norms on $V$. Then $\|\cdot\|_1$ and $\|\cdot\|_2$ are equivalent if and only if the induced metric topologies are equal. 
\end{prp}

\begin{prp}{}{}\\
Let $k$ be a field. Let $V$ be a vector space over $k$. Then all norms on $V$ are equivalent. 
\end{prp}

\subsection{Riesz's Lemma}
\begin{prp}{}{}\\
Let $k$ be a field. Let $V$ be a finite dimensional normed space. Let $U\subseteq V$ be a subset. Then $U$ is compact if and only if $U$ is closed and bounded. 
\end{prp}
...
\begin{defn}{Topological Field}{}\\
Let $k$ be a field equipped with a topology. We say that $k$ is a topological field if the following are true. 
\begin{itemize}
\item The addition map $+:k\times k\to k$ is continuous. 
\item The multiplication map $\cdot:k\times k\to k$ is continuous. 
\item The inverse map $(\cdot)^{-1}:k^\times\to k^\times$ is continuous. 
\end{itemize}
\end{defn}

\begin{defn}{Topological Vector Space}{}\\
Let $k$ be a topological field. Let $V$ be a vector space over a field $k$ that is also a topological space. We say that $V$ is a topological vector space if the following are true. 
\begin{itemize}
...
\begin{lmm}{}{}\\
Let $k$ be a topological field. Let $V$ be a normed space over $k$. Then the metric topology induced by the induced metric from the norm equips $V$ with the structure of a topological vector space. 
\end{lmm}

Let $k$ be a topological field. Let $V$ be a vector space. Then any norm on $V$ makes $V$ a topological vector space. Moreover, by the above proposition, there is a uniquely defined topology on $V$ as long as at least one norm on $V$ exists. 

\subsection{The Topology of Normed Spaces over $\R$}
\begin{prp}{}{}\\
Let $V$ be a finite dimensional normed space over $\R$. Then the induced metric on $V$ is complete. 
\end{prp}

\begin{prp}{}{}\\
Let $V,W$ be finite dimensional normed spaces over $\R$. Let $T:V\to W$ be a linear map. Then $T$ is continuous. 
\end{prp}
