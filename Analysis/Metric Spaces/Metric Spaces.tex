...
\pagebreak
\section{Limits and Continuity in Metric Spaces}
...
Let $X$ be a metric space. We say that $X$ is disconnected if there exists open subsets $\emptyset\neq U,V\subseteq X$ such that $X=U\cup V$. We say that $X$ is connected otherwise. 
\end{defn}
...
\begin{prp}{}{} Let $X$ be a metric space. Let $S\subset X$ be a metric subspace. Then $S$ is connected if and only if the following is true. If $U,V$ are open subsets of $X$ and $U\cap V\cap S=\emptyset$ and $S\subseteq U\cup V$ implies $S\subseteq U$ or $S\subseteq V$. 
\begin{proof}
\end{proof}
\end{prp}
...
\begin{lmm}{}{} Let $X$ be a metric space. The countable union of connected subsets of $X$ such that they have a nonempty intersection is connected. 
\begin{proof}
...
\begin{prp}{}{} Continuity preserves connectedness. That is, if $f:X\to Y$ is a continuous function between metric spaces and $X$ is connected, then $f(X)$ is conneted. 
\begin{proof}
...
\pagebreak
\section{Complex Numbers as a Metric Space}
\subsection{Metric Properties of $\C$}
\begin{defn}{The Standard Norm on $\C$}{}\\
Define the standard norm on $\C$ to be the function $\abs{\cdot}:\C\to\R$ by $$\abs{a+bi}=\sqrt{a^2+b^2}$$
\end{defn}

\begin{lmm}{}{}\\
The standard norm on $\C$ is indeed a norm on $\C$. 
\end{lmm}

The norm thus induces a metric $d:\C\times\C\to\R$ on $\C$ given by $$d(z_1,z_2)=\abs{z_1-z_2}$$ Thus it makes sense to talk about sequences, limits and continuity of functions from $\C$ to $\C$. Since $\C$ is also a field, we would like to know how addition and multiplication interacts with sequences and limits. 
