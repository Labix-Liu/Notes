...
\subsection{The Interior, Boundary and Closure Operators}
\begin{defn}{Interior and Boundary Points}{}\\
Let $X$ be a metric space. Let $U\subseteq X$ be a subset. Let $x\in X$. 
\begin{itemize}
\item We say that $x$ is an interior points of $U$ if there exists $r>0$ such that $B_r(x)\subset U$. 
\item We say that $x$ is a boundary point of $U$ if for all $r>0$, we have $$B_r(x)\cap U\neq\emptyset\neq B_r(x)\cap(X\setminus U)$$
\end{itemize}
\end{defn}

\begin{defn}{The Interior, Boundary and Closure Operator}{}\\
Let $X$ be a metric space. Let $U\subseteq X$ be a subset. 
\begin{itemize}
\item Define the interior of $U$ to be $$U^\circ=\{x\in U\;|\;x\text{ is an interior point of }U\}$$
\item Define the boundary of $U$ to be $$\partial U=\{x\in U\;|\;x\text{ is a boundary point of }U\}$$
\item Define the closure of $U$ to be $$\overline{U}=U\cup\partial U$$
\end{itemize}
\end{defn}
...
\begin{itemize}
\item $U$ is open if and only if $U=U^\circ$ if and only if $U\cap\partial U=\emptyset$. 
\item $U$ is closed if and only if $U=\overline{U}$. 
\end{itemize}
\begin{proof}
\end{proof}
\end{prp}
...
Let $M$ be a metric space. Let $x\in M$. We say that $x$ is an accumulation point of $M$ if for all $r\in\R^+$, we have $$B_r(x)\setminus\{p\}\neq\emptyset$$
\end{defn}
