...
\begin{prp}{}{} If $K$ is algebraic over $L$ and $L$ is algebraic over $F$ then $K$ is algebraic over $F$. 
\end{prp}
...
\begin{eg}{}{}\\
The splitting field of $x^3-2\in\Q[x]$ is the field $\Q(2^{1/3},e^{2\pi i/3})$. 
\end{eg}
...
\begin{prp}{}{} Let $F$ be a field such that $\text{char}(F)=p>0$. Let $f\in F[x]$ be an irreducible polynomial. Then $f$ is inseparable over $K$ if and only if $f$ is of the form $$f(x)=\sum_{k=0}^na_kx^{kp}$$
\end{prp}
...
Let $K/F$ be a field extension. Define the separable closure of $F$ to be $$S_{K/F}=\{\alpha\in K\;|\;\alpha\text{ is separable over }F\}$$
\end{defn}
...
\begin{defn}{Galois Extension}{} Let $K/F$ be an algebraic field extension. We say that $K/F$ is a Galois extension if $K$ is the splitting field of a separable polynomial with coefficients in $F$. 
\end{defn}
