...
Let $R$ be a commutative ring. Recall that if $R$ is a local ring or if $R$ is graded, then any finitely generated free (graded) modules that are projective must be free. Thus projective resolutions and free resolutions of these rings amount to the same thing. 

\begin{defn}{Minimal Free Resolutions}{}\\
\begin{itemize}
\item Let $(R,m)$ be a local ring. Let $M$ be a finitely generated $R$-module. Let $\mF$ be a free resolution of $M$. We say that the resolution is minimal if the differentials in $\mF\otimes_R R/m$ are all $0$ and $F_0\otimes_RR/m\to M\otimes_RR/m$ is an isomorphism. 
\item Let $R$ be a commutative $\N$-graded ring such that $R_0$ is a field. Let $M$ be a graded $R$-module. Let $\mF$ be a free resolution of $M$. We say that the resolution is minimal if the differentials in $\mF\otimes_RR_0$ are all $0$ and $F_0\otimes_RR_0\to M\otimes_RR_0$ is an isomorphism. 
\end{itemize}
\end{defn}

TBA: Any two minimal resolutions are isomorphic. 

\begin{prp}{}{}\\
\begin{itemize}
\item Let $(R,m)$ be a local ring. Let $\mF=(F,d)$ be a resolution of $m$. Then $\mF$ is minimal if and only if for each $n\in\N$, a basis of $F_n$ maps onto a minimal set of generators of $\text{coker}(d_{n+1})$. 
\item Let $R$ be an $\N$-graded ring such that $R_0$ is a field. Let $\mF=(F,d)$ be a resolution of $R$. Then $\mF$ is minimal if and only if for each $n\in\N$, the dimension of $F_n$ as a $k$-vector space is minimal. 
\end{itemize}
\end{prp}

TBA: The Koszul complex is a minimal free resolution. 
