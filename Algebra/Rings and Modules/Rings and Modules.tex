...
Let $R$ be a ring. Let $A,B$ be $R$-modules. Elements of the tensor product are denoted by $a\otimes_R b$ for $a\in A$ and $b\in B$. 
...
\end{itemize}
\begin{proof}\\
Define $\Z$-module homomorphism $f:\Z/m\Z\otimes\Z/n\Z\to\Z/\gcd(m,n)\Z$ by $a\otimes_\Z b\mapsto ab\;(\bmod\;\gcd(m,n))$. Clearly $f$ is surjective since $1\otimes 1\mapsto 1$. We also show that $f$ is injective. Suppose that $ab$ is divisible by $\gcd(m,n)$. Then there exists $k\in\N$ such that $ab=\gcd(m,n)k$. Then $a\otimes_\Z b=ab(1\otimes_\Z1)=\gcd(m,n)k(1\otimes_\Z1)$. By Bezout's lemma, there exists $x,y\in\Z$ such that $$a\otimes_\Z b=\gcd(m,n)k(1\otimes_\Z1)=(mx+ny)k(1\otimes_\Z1)=xk(m\otimes_\Z 1)+yk(1\otimes_\Z n)=0+0=0$$ Hence $f$ is injective. 
\end{proof}
\end{prp}
...
Thus free modules are precisely direct sums of copies of the base ring $R$. 

\begin{prp}{Universal Property of Free Modules}{}\\
Let $R$ be a ring. Let $B$ be a set. Then the free $R$-module $\bigoplus_{b\in B}Rb$ satisfies the following universal property. For every $R$-module $N$ and every map of sets $g:B\to N$, there exists a unique $R$-module homomorphism $h:\bigoplus_{b\in B}Rb\to N$ such that the following diagram commutes: \\~\\
\adjustbox{scale=1.0,center}{\begin{tikzcd}
	B & \bigoplus_{b\in B}Rb \\
	& N
	\arrow["f", from=1-1, to=1-2]
	\arrow["g"', from=1-1, to=2-2]
	\arrow["{\exists!}", dashed, from=1-2, to=2-2]
\end{tikzcd}}\\~\\
Moreover, $\bigoplus_{b\in B}Rb$ is the unique (up to unique isomorphism) $R$-module satisfying the above. 
\end{prp}

Thus to test whether a given $R$-module is free, it suffices to find a basis for the module in which the above universal property holds. 

\begin{eg}{}{} Let $R$ be a commutative ring. Then the following are true. 
\begin{itemize}
...
\begin{prp}{}{} Let $R$ be a division ring. Let $M$ be a left $R$-module. Then the following are true. 
\begin{itemize}
...
\begin{eg}{}{}\\
Let $R$ be a commutative ring. Then there is an isomorphism of $R$-algebras $$\Lambda\left(\bigoplus_{i=1}^nRx_i\right)\cong\frac{R[x_1,\dots,x_n]}{(x_ix_j+x_jx_i,x_i^2\;|\;1\leq i\neq j\leq n)}$$ It is common to denote this $R$-algebra by $\Lambda_R[x_1,\dots,x_n]$. 
\end{eg}

\begin{lmm}{}{}\\
Let $k$ be a field. Let $V$ be a vector space over $k$ of dimension $n$. Then the following are true. 
\begin{itemize}
\item Let $B$ be a basis of $V$. Then $$\{v_{i_1}\wedge\cdots\wedge v_{i_r}\;|\;v_{i_k}\in B\text{ and }1\leq i<\cdots<i_r\leq n\}$$ forms a basis for $\Lambda^r(V)$. 
\item The dimension of the exterior power is given by $$\dim(\Lambda^r(V))=\binom{n}{r}$$
\end{itemize}
\end{lmm}
