...
\begin{defn}{The Words of a Set}{}\\
Let $S$ be a set. Let $T=S\times\{-1,1\}$. A word of $S$ of length $n\in\N\setminus\{0\}$ is a map of sets $\omega:\{1,\dots,n\}\to T$. Such a word is denoted by $$s_1\cdots s_n$$ where $s_k=\omega(k)$. Define the empty word to be the unique map $\emptyset\to T$. 
\end{defn}

We think of $S\times\{1\}$ as the original copy of $S$, and $S\times\{-1\}$ as formally adjoining inverse elements so that we can form a group. 

\begin{defn}{Reduced Words}{}\\
Let $S$ be a set. Define the projection maps $p_S:S\times\{-1,1\}\to S$ and $p_{\{-1,1\}}:S\times\{-1,1\}\to\{-1,1\}$ by $p_S(s,i)=s$ and $p_{\{-1,1\}}(s,i)=i$ respectively. Let $s=s_1\cdots s_n$ be a word of length $n$. We say that $s$ is reduced if for $1\leq i<n$, $p_S(s_i)=p_S(s_{i+1})$ implies that $p_{\{-1,1\}}(s_i)=p_{\{-1,1\}}(s_{i+1})$. By convention the empty word is reduced. 
\end{defn}

Intuitively, since $(s,-1)$ for $s\in S$ represents the inverse of $(s,1)$, saying that $s_1\cdots s_n$ is reduced means that no element is next to its inverse. 

\begin{defn}{Set of Reduced Words}{}\\
Let $S$ be a set. Define the set of reduced words by $$F_S=\{w\;|\;w\text{ is a reduced word of }S\}$$
\end{defn}

\begin{prp}{}{}\\
Let $S$ be a set. Recursively define a product $\ast:F_S\times F_S\to F_S$ on the set of reduced words as follows $$(s_1\cdots s_m)\ast(t_1\cdots t_n)=\begin{cases}
s_1\cdots s_{m-1}\ast t_2\cdots t_n & \text{ if }p_S(s_m)=p_S(t_1)\text{ and }p_{\{0,1\}}(s_m)\neq p_{\{0,1\}}(t_1)\\
...
\begin{defn}{The Free Group on a Set}{}\\
Let $S$ be a set. Define the free group on $S$ to be the set of reduced words $$F_S=\{w\;|\;w\text{ is a reduced word of }S\}$$ together with the binary operation given by the above product. 
\end{defn}

\begin{prp}{Universal Property of Free Groups}{}\\
Let $S$ be a set. Then the free group $F_S$ and the inclusion map $\iota:S\to F_S$ satisfies the following universal property. For any group $G$ and any map of sets $\psi:S\to G$, there exists a unique group homomorphism $\phi:F_S\to G$ such that the following diagram commutes: \\
...
\begin{lmm}{}{}\\
Let $S$ be a set. Then $S$ is a generating set of the free group $F_S$ on $S$. 
\end{lmm}

\begin{defn}{Free Group of Rank n}{}\\
Let $n\in\N$. Define the free group of rank $n$ to be the free group $F_n$ on the set $\{1,\dots,n\}$. 
\end{defn}

\begin{prp}{}{}\\
Let $G$ be a group. Then there exists a set $S$ and a normal subgroup $N$ of $F_S$ such that there is a group isomorphism $G\cong F_S/N$. 
\end{prp}

\subsection{Free Products of Groups}
\begin{defn}{The Words of a Collection of Groups}{}\\
Let $\{G_i\;|\;i\in I\}$ be a collection of groups. A word on these groups is a finite sequence $g_1\cdots g_m$ where $g_k\in G_i$ for some $i\in I$. We say that such a word has length $m$. 
\end{defn}

\begin{defn}{Reduced Words}{}\\
Let $\{G_i\;|\;i\in I\}$ be a collection of groups. Let $g_1\cdots g_m$ be a word. The word is said to be reduced if the following are true. 
\begin{itemize}
...
\begin{prp}{}{}\\
Let $\{G_i\;|\;i\in I\}$ be a collection of groups. Recursively define a product on the set of reduced words as follows. $$(g_1\cdots g_m)\ast(h_1\cdots h_n)=\begin{cases}
...
\begin{defn}{The Free Product of Groups}{}\\
Let $\{G_i\;|\;i\in I\}$ be a collection of groups. Define the free product of groups to be the set $$\ast_i G_i=\{w\;|\;w\text{ is a reduced word of }\{G_i\;|\;i\in I\}\}$$ together with the above product $\ast$. 
\end{defn}

Note: If $\{G_i\}$ is just a collection of two groups then we write the free product as $G_1\ast G_2$. \\

\begin{prp}{}{}\\
Let $\{G_i\;|\;i\in I\}$ be a collection of groups. Then $G_k\leq\ast_iG_i$ for any $k\in I$. 
\begin{proof}
...
Let $G$ be a group. Let $S\subseteq G$ be a subset of $G$. Define the normal closure of $S$ in $G$ to be the subgroup $$\langle S^G\rangle=\langle g^{-1}sg\;|\;\text{ for all }s\in S\text{ and }g\in G\rangle$$
\end{defn}

\begin{lmm}{}{}\\
Let $G$ be a group. Let $S\subseteq G$ be a subset of $G$. Then $\langle S^G\rangle$ is a normal subgroup of $G$. 
\end{lmm}
...
\begin{eg}{}{}\\
Let $n\geq 3$. Then there is a group isomorphism $$D_{2n}\cong\langle r,s\;|\;r^n, s^2, rsrs\rangle$$
\end{eg}
...
\begin{eg}{}{}\\
Let $G,H$ be groups. Then $G\ast_{\{1\}} H=G\ast H$. 
\end{eg}

\begin{lmm}{}{}\\
Let $G,H,K$ be groups. Let $\phi:K\to G$ and $\psi:K\to H$ be injective group homomorphisms. Then $G,H$ are subgroups of $G\ast_KH$. 
\end{lmm}

\begin{prp}{Universal Property of The Free Product}{}\\
Let $G,H,K$ be groups. Let $\phi:K\to G$ and $\psi:K\to H$ be injective group homomorphisms. Then the amalgamated product $G\ast_KH$ satisfies the following universal property. For group $A$ and any group homomorphism $p:G\to A$ and $q:H\to A$ such that $p\circ\phi=q\circ\psi$, there exists a unique homomorphism $$G\ast_KH\to A$$ such that the following diagram commutes: \\
\adjustbox{scale=1.0,center}{\begin{tikzcd}
	K & G & \\
	H & {G\ast_KH} \\
	&& A
	...
	\arrow["{\text{incl}}", hook, from=1-2, to=2-2]
	\arrow["p", bend left = 20, from=1-2, to=3-3]
	\arrow["{\text{incl}}", hook, from=2-1, to=2-2]
	\arrow["q"', bend right = 20, from=2-1, to=3-3]
	\arrow["{\exists!}", dashed, from=2-2, to=3-3]
\end{tikzcd}} \\
Moreover, $G\ast_KH$ is the unique group (up to unique isomorphism) that has such a property. 
\end{prp}
...
\begin{defn}{Torsion Elements}{}\\
Let $G$ be a group. Let $g\in G$. We say that $g$ is a torsion element if $\abs{g}$ is finite. 
\end{defn}

\begin{defn}{Torsion Group}{}\\
Let $G$ be a group. We say that $G$ is a torsion group if all elements of $G$ are torsion. 
\end{defn}
...
\begin{defn}{Torsion Free Group}{}\\
Let $G$ be a group. We say that $G$ is torsion free if the only torsion element is $1_G$. 
\end{defn}

\begin{defn}{Torsion Subgroup}{}\\
Let $G$ be an abelian group. Define the torsion subgroup of $G$ to be $$G^\text{tor}=\{g\in G\;|\;g\text{ is torsion }\}$$
\end{defn}

If $G$ is not abelian, then $G^\text{tor}$ is not a group. 

\begin{lmm}{}{}\\
Let $G$ be a an abelian group. Then the following are true. 
\begin{itemize}
\item $G^\text{tor}$ is a torsion group. 
\item $G/G^\text{tor}$ is a torsion free group. 
\end{itemize}
\end{lmm}

\subsection{p-groups}
\begin{defn}{$p$-groups}{}\\
Let $G$ be a group. Let $p$ be a prime. We say that $G$ is a $p$-group if for all $g\in G$, we have $\abs{g}=p^n$ for some $n\in\N$. 
\end{defn}

\begin{lmm}{}{}\\
Let $G$ be a finite group. Let $p$ be a prime. Then $G$ is a $p$-group if and only if $\abs{G}=p^n$ for some $n\in\N$. 
\end{lmm}

\begin{prp}{}{}\\
Let $p$ be a prime. Let $G$ be a non-trivial finite $p$-group. Then $\abs{Z(G)}>1$. 
\end{prp}

\begin{prp}{}{}\\
Let $p$ be a prime. Let $G$ be a finite $p$-group of order $\abs{G}=p^n$ for some $n\in\N$. Then for each $0\leq k\leq n$, there exists a normal subgroup $N\trianglelefteq G$ such that $\abs{N}=p^k$. 
\end{prp}

\subsection{Simple Groups}
\begin{defn}{Simple Groups}{}\\
Let $G$ be a group. We say that $G$ is simple if $\{1_G\}$ and $G$ are the only normal subgroups of $G$. 
\end{defn}
...
\begin{eg}{}{}\\
Let $n\geq 3$. Suppose that a presentation of $D_{2n}$ is given by $D_{2n}=\langle r,s\;|\;r^n, s^2, rsrs\rangle$. Then we have $[D_{2n},D_{2n}]=\langle r^2\rangle$. 
\begin{proof}\\
A general element of $D_{2n}$ under the above presentation is of the form $s^ir^j$ for $0\leq i\leq 1$ and $0\leq j\leq n-1$. We compute that $[s^{i_1}r^{j_1},s^{i_2}r^{j_2}]=r^{2(j_1-j_2)}$. Thus the commutator of $D_{2n}$ is given by $\langle r^2\rangle$. 
\end{proof}
\end{eg}
...
\begin{eg}{}{}\\
Let $n\geq 3$. Then the following are true. 
\begin{itemize}
\item If $n$ is odd, then $(D_{2n})^\text{ab}\cong C_2$. 
\item If $n$ is even, then $(D_{2n})^\text{ab}\cong C_2\times C_2$. 
\end{itemize}
\begin{proof}\\
Recall we computed that $[D_{2n},D_{2n}]=\langle r^2\rangle$. If $n$ is odd, then $\langle r^2\rangle=\langle r\rangle$ and $\abs{(D_{2n})^\text{ab}}=2$. The only possible group for the abelianization is $C_2$ in this case. \\

If $n$ is even, consider the homomorphism $D_{2n}\to C_2\times C_2$ given by $r\mapsto(1,0)$ and $s\mapsto(0,1)$. This is well defined by the universal property of the group of generators and relations. The kernel of this map is precisely $\langle r^2\rangle$. By the first isomorphism theorem, we have a map $(D_{2n})^\text{ab}\to C_2\times C_2$ that is surjective. Hence it is an isomorphism. 
\end{proof}
\end{eg}
...
\begin{prp}{Universal Property of Abelianization}{}\\
Let $G$ be a group. Then the abelianization $G^\text{ab}$ of $G$ together with the quotient homomorphism $G\to G^\text{ab}$ satisfies the following universal property: For any abelian group $A$ and any group homomorphism $\psi:G\to A$, there exists a unique group homomorphism $\phi:G^\text{ab}\to A$ such that the following diagram commutes: \\
\adjustbox{scale=1.0,center}{\begin{tikzcd}
	G & G^\text{ab} \\
	& A
	...
	\arrow["{\exists!\phi}", dashed, from=1-2, to=2-2]
\end{tikzcd}} \\
Moreover, $G^\text{ab}$ is the unique group (up to unique isomorphism) that has such a property. 
\end{prp}

\begin{defn}{Perfect Groups}{}\\
Let $G$ be a group. We say that $G$ is perfect if $[G,G]=G$. 
\end{defn}

\begin{lmm}{}{}\\
Let $G$ be a group. If $G$ is simple, then $G$ is perfect. 
\begin{proof}\\
Follows from the fact that $[G,G]\trianglelefteq G$ is a normal subgroup and $G$ is simple. 
\end{proof}
\end{lmm}
...
\pagebreak
\section{Some Classification Results of Finite Groups}
...
\subsection{Recognizing Dihedral Groups}
With the inversion homomorphism, the following proposition shows that $D_{2n}$ can be decomposed into a semidirect product. 

\begin{prp}{}{}\\
Let $\sigma=\begin{pmatrix}1 & \cdots &  n\end{pmatrix}\in S_n$ and $\tau=\prod_{i=1}^{\floor{n/2}}\begin{pmatrix}i & n-i+1\end{pmatrix}\in S_n$. Write $K=\langle\sigma\rangle$ and $H=\langle\tau\rangle$. Then $$D_{2n}\cong H\ltimes_\phi K$$ where $\phi:H\to\text{Aut}(K)$ is the inversion homomorphism. 
\begin{proof}
...
\subsection{Groups of Order up to 12}
In this section we will list group from increasing order. 
...
We provide a table for the classification of finite groups up to order $12$: 

\begin{center}
\begin{tabular}{c l c}
\hline
$n$ & Groups of order $n$ & Number \\
\hline
1 & $\{1\}$ & 1 \\
2 & $C_2$ & 1 \\
3 & $C_3$ & 1 \\
...
6 & $C_6,\ D_3$ & 2 \\
7 & $C_7$ & 1 \\
8 & $C_8,\ C_4\times C_2,\ C_2\times C_2\times C_2,\ D_8,\ Q_8$ & 5\\
9 & $C_9,\ C_3\times C_3$ & 2\\
10 & $C_{10},\ D_{10}$ & 2\\
11 & $C_{11}$ & 1\\
12 & $C_{12},\ C_6\times C_2,\ A_4,\ D_{12},\ \text{Dic}_{12}$ & 5\\
\hline
\end{tabular}
\end{center}
...
Let $G$ be a group. Define the lower central series $\gamma_n(G)$ as follows. 
\begin{itemize}
\item $\gamma_0(G)=G$. 
\item $\gamma_{n+1}(G)=[\gamma_n(G),G]$. 
\end{itemize}
\end{defn}

\begin{defn}{Upper Central Series}{}\\
Let $G$ be a group. Define the upper central series $Z_n(G)$ as follows. 
\begin{itemize}
\item $Z_0(G)=\{1_G\}$. 
\item $Z_{n+1}(G)=\{x\in G\;|\;[x,y]\in Z_n\text{ for all }y\in G\}$. 
\end{itemize}
\end{defn}
...
\begin{itemize}
\item $G$ is nilpotent. 
\item The lower central series terminates at $\{1\}$. 
\item The upper central series terminates at $G$. 
\end{itemize}
\end{prp}
...
\item If $G$ is a $p$-group, then $G$ is nilpotent. 
\end{itemize}
\end{lmm}

\begin{prp}{}{}\\
Let $G$ be a finite group of order $p_1^{k_1}\cdots p_n^{k_n}$. Let $P_i\in\text{Syl}_{p_i}(G)$. Then the following are equivalent. 
\begin{itemize}
\item $G$ is nilpotent. 
\item If $H<G$ is a proper subgroup, then $H<N_G(H)$ is a proper subgroup. 
\item $P_i\trianglelefteq G$ is a normal subgroup. 
\item $G\cong\prod_{i=1}^nP_i$. 
\end{itemize}
\end{prp}
