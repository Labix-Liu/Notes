...
\fancyhf{}
\rhead{Labix}
\lhead{Introduction to Category Theory}
\rfoot{\thepage}
...
\begin{itemize}
\item A collection $\text{Obj}\mC$ called the objects of $\mC$. 
\item For every $C,D\in\text{Obj}(\mC)$, a collection $\Hom_\mC(C,D)$ called the morphisms from $C$ to $D$. A morphism $f\in\Hom_\mC(C,D)$ is denoted by $f:C\to D$. We call $C$ the source, $D$ the target. 
...
We write $$\mC=\left(\text{Obj}(\mC),\Hom_\mC=\bigcup_{C,D\in\text{Obj}(\mC)}\Hom_\mC(C,D),\circ\right)$$ to abbreviate notation. Sometimes we write $\Hom_\mC(C,D)=\Hom(C,D)$ if context is clear. 

\begin{defn}{Small Categories}{}\\
Let $\mC$ be a category. 
\begin{itemize}
\item We say that $\mC$ is small if the collection of all morphisms form a set. 
\item We say that $\mC$ is locally small if for all $C,D\in\mC$, $\Hom_\mC(C,D)$ is a set. 
\end{itemize}
\end{defn}
...
\begin{defn}{Types of Morphisms}{}\\
Let $\mC$ be a category. Let $f:C\to D$ be a morphism in $\mC$. We say that $f$ is a 
\begin{itemize}
...
\begin{itemize}
\item $\text{Obj}(\mC^{\text{op}})=\text{Obj}(\mC)$
...
\begin{lmm}{}{}\\
Let $\mC$ be a category and $f:C\to D$ be a morphism. Then the following are equivalent. 
\begin{itemize}
\item $f$ is an isomorphism
\item For all $E\in\text{Obj}(\mC)$, the map of sets $$f_\ast:\Hom_\mC(E,C)\to\Hom_\mC(E,D)$$ defined by $g\mapsto f\circ g$ is a bijection
\item For all $E\in\text{Obj}(\mC)$, the map of sets $$f^\ast:\Hom_\mC(D,E)\to\Hom_\mC(C,E)$$ defined by $g\mapsto g\circ f$ is a bijection. 
\end{itemize} 
\begin{proof}~\\
\begin{itemize}
...
\begin{itemize}
\item A subcollection $\text{Obj}(\mD)\subseteq\text{Obj}(\mC)$ of objects
\item For each $C,D\in\text{Obj}(\mD)$, a subcollection $\Hom_\mD(C,D)\subseteq\Hom_\mC(C,D)$ closed under composition, and containing the identities of each objects in $\text{Obj}(\mD)$
\end{itemize}
We say that $\mD$ is a full subcategory of $\mC$ if $\Hom_\mD(C,D)=\Hom_\mC(C,D)$ for all $C,D\in\text{Obj}(\mD)$
\end{defn}
...
\begin{itemize}
\item The object part of $F$ where $F:\text{Obj}(\mC)\to\text{Obj}(\mD)$
\item The arrow part of $F$ where $F:\Hom_\mC(C,D)\to\Hom_\mD(F(C),F(D))$ for all $C,D\in\text{Obj}(\mC)$ satisfying:
\begin{itemize}
\item For all $C\overset{f}{\rightarrow}D\overset{g}{\rightarrow}E$ in $\mC$, we have $$F(g\circ_\mC f)=F(g)\circ_\mD F(f)$$
\item For all $C\in\text{Obj}(\mC)$, $$F(\text{id}_C)=\text{id}_{F(C)}$$
\end{itemize}
\end{itemize}
A contravariant functor is a covariant functor $F:\mC^\text{op}\to\mD$. This consists of 
\begin{itemize}
\item $F:\text{Obj}(\mC)\to\text{Obj}(\mD)$
\item $F:\Hom_{\mC^{\text{op}}}(D,C)\to\Hom_\mD(F(C),F(D))$ for all $C,D\in\text{Obj}(\mC)$ satisfying:
\begin{itemize}
...
\begin{lmm}{}{} Let $F:\mC\to\mD$ be a functor and $f:C\to D$ is an isomorphism in $\mC$. Then $F(f):F(C)\to F(D)$ is an isomorphism in $\mD$. 
\begin{proof}
...
\begin{itemize}
\item On objects, $(G\circ F)(C)=G(F(C))$ for all $C\in\text{Obj}(\mC)$
\item On morphisms, for $f:C\to D$ in $\mC$, $(G\circ F)(f)=G(F(f))$. 
\end{itemize} 
Then $G\circ F$ is also a functor. 
\begin{proof}
...
\begin{itemize}
\item $\text{Obj}\bold{CAT}=$ ''all'' the locally small categories
\item For $\mC,\mD\in\text{Obj}\text{CAT}$, define $\Hom_{\bold{CAT}}(\mC,\mD)=\{\text{Functors }F:\mC\to\mD\}$
...
\begin{itemize}
\item $\text{Obj}(\bold{Set})=\text{''all'' sets}$
...
\begin{prp}{}{} Let $\mC$ be a category and $C$ an object. Then the two categories $$(\mC/C)^\text{op}=C/(\mC^\text{op})$$ are equal. 
\begin{proof}
...
\begin{defn}{Product Categories}{}\\
Let $\mC,\mD$ be categories. Define the product $$\mC\times\mD$$ of $\mC$ and $\mD$ as follows. 
\begin{itemize}
\item The objects are pairs $(C,D)$ for $C\in\mC$ and $D\in\mD$. 
\item For two objects $(C_1,D_1)$ and $(C_2,D_2)$, a morphism from $(C_1,D_1)$ and $(C_2,D_2)$ is a pair $(f,g)$ where $f:C_1\to C_2$ is a morphism in $\mC$ and $g:D_1\to D_2$ is a morphism in $\mD$. 
\item For two morphisms $(f_1,g_1):(C_1,D_1)\to(C_2,D_2)$ and $(f_2,g_2):(C_2,D_2),\to(C_3,D_3)$, composition is defined pairwise. This means that $$(f_2,g_2)\circ(f_1,g_1)=(f_2\circ f_1,g_2\circ g_1)$$
\end{itemize}
\end{defn}
...
\begin{lmm}{}{} Let $F,G:\mC\to\mD$ be functors and $\alpha:F\to G$ be a natural isomorphism. Then the inverses $\alpha^{-1}_C:G(C)\to F(C)$, for $C\in\text{Obj}(\mC)$ defines a natural isomorphism $\alpha^{-1}:G\to F$. 
\begin{proof}
...
\begin{prp}{}{} A functor $F:\mathcal{C}\to\mathcal{D}$ defines an equivalence of categories if and only if $F$ is fully faithful and essentially surjective. 
\begin{proof}
...
&=f:C\to C'
\end{align*}
Similarly $F(\nu_\ast(G))$ is the identity map. Thus $F$ is fully faithful. $F$ is also essentially surjective. For all $D\in\text{Obj}(\mD)$, we can choose $G(D)$ such that $$F(G(D))\cong D$$ by the naturality of $\tau$. \\

Now suppose that $F$ is fully faithful and essentially surjective. Define $G:\mD\to\mC$ as follows: 
\begin{itemize}
\item For every $D\in\text{Obj}(\mD)$, choose $C_D\in\text{Obj}(\mC)$ and isomorphism $\tau_D:F(C_D)\cong D$. This is possible since $F$ is essentially surjective and by the axiom of choice. 
...
\begin{prp}{Vertical Composition}{} Let $F,G,H:\mC\to\mD$ be functors and let $\lambda:F\Rightarrow G$ and $\tau:G\Rightarrow H$ be natural transformations. For each object $C\in\mC$, the collection of morphisms $$\tau_C\circ\lambda_C:F(C)\to H(C)$$ assemble into a natural transformation denoted $$\tau\circ\lambda:F\Rightarrow H$$ 
\begin{proof}
...
\begin{prp}{Horizontal Composition}{} Let $F_1,F_2:\mC\to\mD$ and $G_1,G_2:\mD\to\mE$ be functors such that there are natural transformations $\lambda:F_1\Rightarrow F_2$ and $\tau:G_1\Rightarrow G_2$. Then there exists a natural transformation from $G_1\circ F_1$ to $G_2\circ F_2$. 
\begin{proof}
...
\begin{defn}{Hom Functor in the Second Variable}{} Let $\mC$ be a locally small category. For every $C\in\text{Obj}(\mC)$, define the Hom functor to be $$\Hom_\mC(C,-):\mC\to\text{Set}$$ where 
\begin{itemize}
...
\begin{defn}{Hom Functor in the First Variable}{} Let $\mC$ be a locally small category. For every $C\in\text{Obj}(\mC)$, define the Hom functor to be $$\Hom_\mC(-,C):\mC^\text{op}\to\text{Set}$$ where 
\begin{itemize}
\item On objects, sends $D\in\text{Obj}(\mC)$ to the set $\Hom_\mC(D,C)=\Hom_{\mC^{\text{op}}}(C,D)$
...
\begin{thm}{Yoneda's Lemma I}{} Let $\mC$ be a locally small category. Let $F:\mC\to\bold{Set}$ be a functor. Let $C\in\mC$ be an object. Then there is a natural bijection of sets $$\Hom_{\text{Func}(\mC,\bold{Set})}(\Hom_\mC(C,-),F)\cong F(C)$$ defined by $\left(\alpha:\Hom_\mC(C,-)\Rightarrow F\right)\mapsto\left(\alpha_C(\text{id}_C)\right)$. 
\begin{proof}
...
\begin{prp}{Yoneda's Embedding}{} Let $\mC$ be a locally small category. Then Yoneda's embedding $y:\mC^\text{op}\to\text{Func}(\mC,\bold{Set})$ is fully faithful. Moreover, two objects $C,D$ in $\text{Obj}(\mC)$ are isomorphic if and only if the functors $\Hom_\mC(C,-)$ and $\Hom_\mC(D,-)$ are naturally isomorphic. 
\begin{proof}
...
\begin{crl}{}{} Let $\mC$ be a locally small category. Let $C,D\in\mC$ be objects. Then $C$ and $D$ are isomorphic objects if and only if the functors $\Hom_\mC(C,-)$ and $\Hom_\mC(D,-)$ are naturally isomorphic. 
\begin{proof}
...
Let $\mathcal{C}$ be a category. Let $X\in\mC$. 
\begin{itemize}
\item We say that $X$ is initial if for any $C\in\mC$, there is a unique morphism $X\to C$. 
\item We say that $X$ is terminal if for any $C\in\mC$, there is a unique morphism $C\to X$. 
\item We say that $X$ is a zero object if $X$ is both initial and terminal. 
\end{itemize}
\end{defn}
...
\begin{prp}{}{} Let $\mC$ be a category. Then initial and terminal objects (if it exists) are unique up to unique isomorphism. 
\begin{proof}
...
\begin{defn}{Constant Functor}{} Let $\mJ,\mC$ be categories. The constant functor with value $C\in\text{Obj}(\mC)$ is the functor $\Delta C:\mJ\to\mC$ defined by 
\begin{itemize}
\item For each object $J\in\mJ$, we have $\Delta C(J)=C$. 
\item For each morphism $f:I\to J$ in $\mJ$, we have $\Delta C(f)=\text{id}_C$. 
\end{itemize}
\end{defn}
...
\begin{defn}{Cone Over and Under}{}\\
Let $X:\mJ\to\mC$ be a commutative diagram. A cone over $X$ with summit $C\in\text{Obj}(\mC)$ is a natural transformation $$\lambda:\Delta C\Rightarrow X$$ from the constant functor to the diagram. Explicitly, $\lambda$ is a collection of morphisms $\lambda_J:C\to X(J)$ for each $J\in\text{Obj}\mJ$ such that \\
\adjustbox{scale=1.0,center}{\begin{tikzcd}
...
\begin{defn}{Limits}{} Let $\mC$ be a category. Let $X:\mJ\to\mC$ be a diagram. A limit of $X$ is an object $$\lim_\mJ X\in\text{Obj}(\mC)$$ together with a natural transformation $\lambda:\Delta(\lim_\mJ X)\Rightarrow X$ which is terminal in the category of cones over $X$. 
\end{defn}
...
\begin{defn}{Cone Under a Diagram}{} Let $X:\mJ\to\mC$ be a commutative diagram. A cone under $X$ with apex $C\in\text{Obj}(\mC)$ is a natural transformation $$\lambda:X\Rightarrow\Delta C$$ from the diagram to the constant functor.Explicitly, $\lambda$ is a collection of morphisms $\lambda_J:C\to X(J)$ for each $J\in\text{Obj}\mJ$ such that \\
\adjustbox{scale=1.0,center}{\begin{tikzcd}
	{X(I)} && {X(J)} \\
	& C
...
\begin{defn}{Colimits}{} Let $\mC$ be a category. Let $X:\mJ\to\mC$ be a diagram. A colimit of $X$ is an object $$\colim_\mJ X\in\text{Obj}(\mC)$$ together with a natural transformation $\lambda:X\Rightarrow\Delta\left(\colim_\mJ X\right)$ which is initial in the category of cones under $X$. 
\end{defn}
...
\begin{prp}{Uniqueness of (Co)Limits}{} Let $\mC$ be a category. Let $X:\mJ\to\mC$ be a diagram. Then the limit and colimit of $X$ is unique up to unique isomorphism, provided that it exists. 
\begin{proof}
...
& X(J) &
\end{tikzcd}}\\
for all $J\in\text{Obj}\mJ$. But this precisely means that every $\delta$ has a unique preimage $u$ by $\Psi$. Thus $\Psi$ is a bijection. \\
...
\begin{prp}{}{}\\
Consider the category of categories $\bold{CAT}$. Let $\mC,\mD$ be objects. Then the product of $\mC$ and $\mD$ is the product category $\mC\times\mD$. 
\end{prp}
...
\begin{defn}{Pullbacks}{}\\
Let $\mC$ be a category. Suppose that $\mJ=\left(1\rightarrow 0\leftarrow 2\right)$. A pullback in $\mC$ is the limit of a diagram $X:\mJ\to\mC$. We denote the pullback by $$X(1)\times_{X(0)}X(2)=\lim_\mJ X$$
\end{defn}

\begin{defn}{Pushouts}{}\\
Let $\mC$ be a category. Suppose that $\mJ=\left(1\leftarrow0\rightarrow2\right)$. A pushout in $\mC$ is the colimit of a diagram $X:\mJ\to\mC$. We denote the pushout by $$X(1)\amalg_{X(0)}X(2)=\text{colim}_\mJ X$$
\end{defn}

In particular, the universal product of limits means that for any $X\in\text{Obj}(\mC)$ together with maps $\lambda_C:X\to C$ and $\lambda_B:X\to B$ such that $f\circ\lambda_C=g\circ\lambda_D$, there exists a unique $u:X\to C\times_DB$ such that the following diagram commutes (Dually, the diagram on the right): \\
\adjustbox{scale=1.0,center}{\begin{tikzcd}
...
\begin{prp}{}{}\\
Let $X,Y,Z\in\bold{Set}$ be sets. Then the following are true regarding pullbacks and pushouts. 
\begin{itemize}
\item Let $f:X\to Z$ and $g:Y\to Z$ be functions. Then the pullback of $f$ and $g$ is given by $$X\times_ZY=\{(x,y)\in X\times Y\;|\;f(x)=g(y)\}$$
\item Let $f:Z\to X$ and $g:Z\to Y$ be functions. Then the pushout of $f$ and $g$ is given by $$X\amalg_ZY=\frac{X\amalg Y}{\sim}$$ where $f(z)\sim g(z)$ for $z\in Z$. 
\end{itemize}
\end{prp}

\begin{prp}{}{}\\
Let $X,Y,Z\in\bold{Top}$ be spaces. Then the following are true regarding pullbacks and pushouts. 
\begin{itemize}
\item Let $f:X\to Z$ and $g:Y\to Z$ be maps. Then the pullback of $f$ and $g$ is given by $$X\times_ZY=\{(x,y)\in X\times Y\;|\;f(x)=g(y)\}$$ together with the subspace topology. 
\item Let $f:Z\to X$ and $g:Z\to Y$ be maps. Then the pushout of $f$ and $g$ is given by $$X\amalg_ZY=\frac{X\amalg Y}{\sim}$$ where $f(z)\sim g(z)$ for $z\in Z$, together with the quotient topology. 
\end{itemize}
\end{prp}

\begin{prp}{}{}\\
Let $G,H,K\in\bold{Grp}$ be groups. Then the following are true regarding pullbacks and pushouts. 
\begin{itemize}
\item Let $f:G\to K$ and $g:H\to K$ be maps. Then the pullback of $f$ and $g$ is given by $$G\times_KH=\{(x,y)\in G\times H\;|\;f(x)=g(y)\}$$ as a subgroup of $G\times H$. 
\item Let $f:K\to G$ and $g:K\to H$ be maps. Then the pushout of $f$ and $g$ is given by the amalgamated product $$G\amalg_KH=G\ast_KH=\frac{G\ast H}{N_{G\ast H}(\{f(k)\cdot(g(k))^{-1}\;|\;k\in K\})}$$
\end{itemize}
\end{prp}

\subsection{Inverse and Direct Limts}
\begin{defn}{Inverse Limits}{}\\
Let $I$ be a poset. Let $\mI$ be the category associated to $I$. Let $\mC$ be category. An inverse limit of $I$ in $\mC$ is the limit of a diagram $X:\mI^\text{op}\to\mC$. We denote the limit in this case as $$\varprojlim_\mI X=\lim_{\mI^\text{op}}X$$
\end{defn}

\begin{defn}{Directed Limits}{}\\
Let $I$ be a poset. Let $\mI$ be the category associated to $I$. Let $\mC$ be category. A directed limit of $I$ in $\mC$ is the limit of a diagram $X:\mI\to\mC$. We denote the limit in this case as $$\varinjlim_\mI X=\lim_\mI X$$
\end{defn}

\begin{prp}{}{}\\
Consider the category $\bold{Set}$ of sets. 
\begin{itemize}
\item Let $X:\N^\text{op}\to\mC$ be a diagram. Then the inverse limit of $X$ is given by $$
\varprojlim_\N X=\left\{(x_i)_{i\in\N}\in\prod_{i\in\N}X(i)\;\bigg{|}\;x_j=f_{ij}(x_i)\text{ for all }i\leq j\right\}$$
\item Let $X:\N\to\mC$ be a diagram. Then the direct limit of $X$ is given by $$
\varinjlim_\N X=\frac{\coprod_{i\in\N}X(i)}{\sim}$$ where $x_i\in X(i)\sim x_j\in X(j)$ if $i<j$ and there exists $k\in\N$ such that $f_{ik}(x_i)=f_{jk}(x_j)$. 
\end{itemize}
\end{prp}
...
The universal property of the equalizer is given as follows: For $A\in\text{Obj}(\mC)$ together with a map $\lambda:A\to C$ for which $f\circ\lambda=g\circ\lambda$, there exists a unique map $u:A\to N$ such that the following diagram commutes: \\
\adjustbox{scale=1.0,center}{\begin{tikzcd}
...
\begin{prp}{}{} The category $\text{Set}$ is complete and cocomplete. 
\begin{proof}
Denote $1$ the one element set. Then since we are working with the category of sets, we have the isomorphism $$L(X)=\lim_\mJ X\cong\Hom_\text{Set}(1,\lim_\mJ X)\cong\text{Cone}(1,X)$$ for any small diagram $X:\mJ\to\mC$. This is equal to $$\lim_\mJ X=\left\{\{x_J\in X(J)\}_{J\in\text{Obj}\mJ}\in\prod_{J\in\text{Obj}\mJ}X(J)\;\bigg{|}\;\forall (f:I\to J)\in\mJ, f_\ast(x_I)=x_J\right\}$$ where the maps of the cones are $\pi_J:\lim_\mJ X\to X(J)$ defined by $\{x_I\}_{I\in\text{Obj}\mJ}\to x_J$. Dually, we have that $$\lim_\mJ X=\frac{\coprod_{J\in\text{Obj}\mJ}X(J)}{\sim}$$ where $\sim$ is the equivalence relation generated by $x_I\in X(I)\sim x_J\in X(J)$ if and only if there exists $f:I\to K$ and $g:J\to K$ in $\mJ$ such that $f_\ast(x_I)=g_\ast(x_J)$. The maps are the inclusions $$\iota_J:X(J)\hookrightarrow\frac{\coprod_{I\in\text{Obj}\mJ X(I)}}{\sim}$$ Note that these are sets since $\mJ$ is small. \\
...
\begin{align*}
f_\ast(\pi_I(\{x_K\}_{K\in\text{Obj}\mJ}))&=f_\ast(x_I)\\
&=x_J\\
&=\pi_J(\{x_K\}_{K\in\text{Obj}\mJ})
\end{align*}
...
& X(I) &
\end{tikzcd}}\\
Let us define $u(y)=\{\alpha_J(y)\}_{J\in\text{Obj}\mJ}$. If this is well defined, it is unique. (Two points in $\ prod_{J\in\text{Obj}\mJ}X(J))$ if and only if they have the same components). We need to see that $u(y)\in L(X)$. Let $f:I\to J$, then $f_\ast(\alpha_I(y))=\alpha_J(y)$ since $\alpha$ is natural. Thus we are done. 
\end{proof}
\end{prp}
...
\begin{thm}{}{} Let $\mC$ be a category with small products, and $X:\mJ\to\mC$ be a small diagram. Define two morphisms $$f,g:\prod_{J\in\text{Obj}\mJ}X_J\to\prod_{(\alpha:J\to I)\in\mJ}X_I$$ as follows. 
\begin{itemize}
...
\end{itemize}
If $\text{Eq}(f,g)$ exists, then there is a canonical isomorphism $$\lim_\mJ X\cong\text{Eq}(f,g)$$ 
\begin{proof}
Define a cone over $X$ by $\pi_I^E:\text{Eq}(f,g)\to X_I$ for all $I\in\text{Obj}\mJ$ by \\
\adjustbox{scale=1.0,center}{\begin{tikzcd}
\text{Eq}(f,g)\arrow[r, "\pi_0"] & \prod_{J\in\text{Obj}\mJ}X_J\arrow[r, "\pi_I"] & X_I
\end{tikzcd}} \\
...
\begin{align*}
\Hom_\mC(D,\text{Eq}(f,g))&\cong\text{Cone}\left(D,\prod_{J\in\text{Obj}\mJ}X_J\overset{f}{\underset{g}{\rightrightarrows}}\prod_{(\alpha:J\to I)\in\Hom\mJ}X_I\right)\\
&=\left\{(a\in\Hom_\mC(D,\prod_{J\in\text{Obj}\mJ}X_J))\;\bigg{|}\;\; f\circ a=g\circ a\right\}\\
&=\left\{a\in\Hom_\mC(D,\prod_{J\in\text{Obj}\mJ}X_J)\;\bigg{|}\;\;\forall\alpha:J\to I\in\mJ, \pi_\alpha\circ f\circ\alpha\cong\pi_\alpha\circ g\circ a\right\}\\
&\cong\left\{\{a_J\in\Hom_\mC(D,X_J)\}_{J\in\text{Obj}\mJ}\;\bigg{|}\;\; a_i=\alpha_\ast\circ a_j\right\}\tag{Universal property of $\prod_{J\in\text{Obj}\mJ}X_J$}\\
&=\text{Cone}(D,X)\\
...
\begin{crl}{}{} The category $\bold{Top}$ of topological spaces is complete and cocomplete. 
\begin{proof}
...
\begin{crl}{}{} The category $\bold{Grp}$ of groups is complete and cocomplete. 
\begin{proof}
...
\pagebreak
\section{Properties of Certain (Co)Limits}
\subsection{Filtered Colimits and Cofiltered Limits}
\begin{defn}{Filtered Diagrams}{}\\
Let $\mC$ be a category. Let $\mJ$ be an indexing category. Let $X:\mJ\to\mC$ be a diagram. We say that $X$ is filtered if the following are true. 
\begin{itemize}
\item $\mJ$ is not empty. 
\item For any $a,b\in\mJ$, there exists $c\in\mJ$ and morphisms $a\to c$ and $b\to c$. 
\item For any $a,b\in\mJ$ and $f,g:a\to b$, there exists $c\in\mJ$ and $h:b\to c$ such that $X(h\circ f)=X(h\circ g)$. 
\end{itemize}
\end{defn}

\begin{defn}{Cofiltered Diagrams}{}\\
Let $\mC$ be a category. Let $\mJ$ be an indexing category. Let $X:\mJ\to\mC$ be a diagram. We say that $X$ is filtered if the following are true. 
\begin{itemize}
\item $\mJ$ is not empty. 
\item For any $a,b\in\mJ$, there exists $c\in\mJ$ and morphisms $c\to a$ and $c\to b$. 
\item For any $a,b\in\mJ$ and $f,g:a\to b$, there exists $c\in\mJ$ and $h:c\to a$ such that $X(f\circ h)=X(g\circ h)$. 
\end{itemize}
\end{defn}

\begin{defn}{Filtered Colimits and Cofiltered Limits}{}\\
Let $\mC$ be a category. Let $\mJ$ be an indexing category. Let $X:\mJ\to\mC$ be a diagram. 
\begin{itemize}
\item We say that the limit $\lim_\mJ X$ is cofiltered if $X$ is cofiltered (if it exists). 
\item We say that the colimit $\colim_\mJ X$ is filtered if $X$ if filtered (if it exists). 
\end{itemize}
\end{defn}
...
Let $\mC$ be a category. Let $\mJ$ be an indexing category. Let $X:\mJ\to\mC$ be a diagram. 
\begin{itemize}
\item If $X$ is cofiltered, then $\lim_\mJ X$ exists. 
\item If $X$ is filtered, then $\colim_\mJ X$ exists. 
\end{itemize}
\end{lmm}

\subsection{Final and Cofinal Diagrams}
\begin{defn}{Initial Diagrams}{}\\
Let $\mJ$ be a category. Let $\mI$ be a subcategory of $\mJ$. We say that $\mI$ is initial in $\mJ$ if the following are true. 
\begin{itemize}
\item For all $y\in\mJ$, there exists $x\in\mI$ and a morphism $f:x\to y$ in $\mJ$. 
\item For all $y\in\mJ$, $x,x'\in\mI$ and $f:x\to y$ and $f':x'\to y$, there exists a sequence of morphisms $$x=x_0\leftarrow x_1\rightarrow\cdots\leftarrow x_{2n-1}\rightarrow x_{2n}=x'$$ in $\mI$ and morphisms $x_k\to y$ in $\mJ$ for $0\leq k\leq 2n$ such that the following diagram commutes: \\
\adjustbox{scale=1.0,center}{\begin{tikzcd}
...
\begin{defn}{Cofinal Diagrams}{}\\
Let $\mJ$ be a category. Let $\mI$ be a subcategory of $\mJ$. We say that $\mI$ is initial in $\mJ$ if the following are true. 
\begin{itemize}
\item For all $x\in\mJ$, there exists $y\in\mI$ and a morphism $f:x\to y$ in $\mJ$. 
\item For all $x\in\mJ$, $y,y'\in\mI$ and $f:x\to y$ and $f':x\to y'$, there exists a sequence of morphisms $$y=y_0\leftarrow y_1\rightarrow\cdots\leftarrow y_{2n-1}\rightarrow y_{2n}=y'$$ in $\mI$ and morphisms $x\to y_k$ in $\mJ$ for $0\leq k\leq 2n$ such that the following diagram commutes: \\
\adjustbox{scale=1.0,center}{\begin{tikzcd}
	& x \\
	{y_{2k}} & {y_{2k+1}} & {y_{2k+2}}
...
Some literature refer to cofinality also as finality. 

\begin{prp}{}{}\\
Let $\mC$ be a category. Let $\mJ$ be an indexing category. Let $X:\mJ\to\mC$ be a diagram. Let $\mI$ be a subcategory of $\mJ$. Then the following are true. 
\begin{itemize}
\item Suppose that the limit of $X$ exists. If $\mI$ is initial in $\mJ$, then we have $$\lim_\mJ X=\lim_\mI X|_\mI$$
\item Suppose that the colimit of $X$ exists. If $\mI$ is cofinal in $\mJ$, then we have $$\colim_\mJ X=\colim_\mI X|_\mI$$
\end{itemize}
\end{prp}

\subsection{Commuting Functors with (Co)Limits}
\begin{defn}{(Co)Limit Preserving Functors}{}\\
Let $\mC,\mD$ be categories. Let $F:\mC\to\mD$ be a functor. Let $X:\mJ\to\mC$ be a diagram. We say that $F$ commutes with the limit of $X$ if there is a natural isomorphism $$F\left(\lim_\mJ X\right)\cong\lim_\mJ F\circ X$$ Dually, we say that $F$ preserves the colimit of $X$ if there is a natural isomorphism $$F\left(\colim_\mJ X\right)\cong\colim_\mJ F\circ X$$
\end{defn}
...
\begin{prp}{}{}\\
Let $\mC$ be a category. Let $\mI$ and $\mJ$ be indexing categories. Let $F:\mI\times\mJ\to\mC$ be a functor. Then the following are true. 
\begin{itemize}
\item There is a natural isomorphism $$\lim_\mJ\lim_\mI F\cong\lim_\mI\lim_\mJ F$$ if all limits exists. 
\item There is a natural isomorphism $$\colim_\mJ\colim_\mI F\cong\colim_\mI\colim_\mJ F$$ if all colimits exists. 
\end{itemize}
\end{prp}
...
\begin{prp}{}{}\\
Let $\mI$ and $\mJ$ be indexing categories. Let $F:\mI\times\mJ\to\bold{Set}$ be a functor. Then the following are true. 
\begin{itemize}
\item If $\mI$ is filtered and $\mJ$ is finite, then there is a natural isomorphism $$\colim_\mI\lim_\mJ F\cong\lim_\mJ\colim_\mI F$$
\item If $\mI$ is cofiltered and $\mJ$ is finite, then there is a natural isomorphism $$\lim_\mI\colim_\mJ F\cong\colim_\mJ\lim_\mI F$$
\end{itemize}
\end{prp}
...
Let $\mC,\mD$ be a categories. Let $L:\mC\to\mD$ and $R:\mD\to\mC$ be functors. We say that $L$ is a left adjoint of $R$ (and $R$ is a right adjoint of $L$) if there exists a natural isomorphism $$a_{-,-}:\Hom_\mD(L(-),-)\overset{\cong}{\to}\Hom_\mC(-,R(-))$$ In this case, we also say that $L$ admits a right adjoint (and $R$ admits a left adjoint). 
\end{defn}
...
\end{tikzcd}} \\
This means that there is a one-to-one correspondence: $$\left\{\substack{a:\Hom_\mD(L(-),-)\overset{\cong}{\longrightarrow}\Hom_\mC(-,R(-))\\a\text{ is a natural isomorphism }}\right\}\;\;\;\;\overset{1:1}{\longleftrightarrow}\;\;\;\;\left\{(\varepsilon,\eta)\;\bigg{|}\;\substack{\eta:\text{id}_\mC\Rightarrow RL\text{ and }\varepsilon:LR\Rightarrow\text{id}_\mD\\\text{ are natural transformations }\\\text{satisfying the triangle conditions}}\right\}$$ 
\begin{proof}
Suppose first that $L\dashv R$. Let $$a_{-,-}:\Hom_\mD(L(-),-)\overset{\cong}{\longrightarrow}\Hom_\mC(-,R(-))$$ be the natural isomorphism witnessing the adjunction. Define two natural transformations $\eta:\text{id}_\mC\Rightarrow R\circ L$ and $\epsilon:L\circ R\Rightarrow\text{id}$ by $\eta_C=a_{C,L(C)}(\text{id}_{F(C)})$ and $\epsilon_D=a_{R(D),D}^{-1}(\text{id}_{R(D)})$. Notice that $\eta$ and $\epsilon$ are natural transformations via the following commutative diagrams: \\
\adjustbox{scale=1.0,center}{\begin{tikzcd}
	{\Hom_\mD(L(C),L(C))} & {\Hom_\mC(C,RL(C))} && {\Hom_\mD(LR(D),D)} & {\Hom_\mC(R(D),R(D))} \\
	\\
	{\Hom_\mD(L(C),L(C'))} & {\Hom_\mC(C,RL(C'))} && {\Hom_\mD(LR(D),D')} & {\Hom_\mC(R(D),R(D'))} \\
	\\
	{\Hom_\mD(L(C'),L(C'))} & {\Hom_\mC(C',RL(C'))} && {\Hom_\mD(LR(D'),D')} & {\Hom_\mC(R(D'),R(D'))}
	\arrow["{a_{C,L(C)}}", from=1-1, to=1-2]
	\arrow["{L(f)\circ -}"', from=1-1, to=3-1]
	\arrow["{RL(f)\circ -}", from=1-2, to=3-2]
	\arrow["{g\circ -}"', from=1-4, to=3-4]
	\arrow["{a_{R(D),D}^{-1}}"', from=1-5, to=1-4]
	\arrow["{R(g)\circ -}", from=1-5, to=3-5]
	\arrow["{a_{C,L(C')}}", from=3-1, to=3-2]
	\arrow["{a_{R(D),D'}^{-1}}"', from=3-5, to=3-4]
	\arrow["{-\circ L(f)}", from=5-1, to=3-1]
	\arrow["{a_{C',L(C')}}"', from=5-1, to=5-2]
	\arrow["{-\circ f}"', from=5-2, to=3-2]
	\arrow["{-\circ LR(g)}", from=5-4, to=3-4]
	\arrow["{-\circ R(g)}"', from=5-5, to=3-5]
	\arrow["{a_{R(D'),D'}^{-1}}", from=5-5, to=5-4]
\end{tikzcd}} \\
Indeed, for naturality of $\eta$, we start at the top left and bottom left of the left diagram with the elements $\text{id}_{L(C)}$ and $\text{id}_{L(C')}$. Since the diagram is commutative, we deduce that $$\eta_{C'}\circ f=a_{C',L(C')}(\text{id}_{L(C')})\circ f=a_{C,L(C')}(L(f))$$ and $$RL(f)\circ\eta_C=RL(f)\circ a_{C,L(C)}(\text{id}_{L(C)})=a_{C,L(C')}(L(f))$$ Since $a_{C,L(C')}$ is an isomorphism, we conclude that $\eta_{C'}\circ f=RL(f)\circ\eta_C$ so that the naturality condition is satisfied. For the naturality of $\epsilon$, we start at the top right and bottom right of the right diagram with the elements $\text{id}_{R(D)}$ and $\text{id}_{R(D')}$ respectively. Since the diagram is commutative, we deduce that $$g\circ\epsilon_D=g\circ\alpha_{R(D),D}^{-1}(\text{id}_{R(D)})=a_{R(D),D'}(R(g))$$ and $$\epsilon_{D'}\circ LR(g)=a^{-1}(\text{id}_{R(D')})\circ LR(g)=a_{R(D),D'}(R(g))$$ Since $a_{R(D),D'}$ is an isomorphism, we conclude that $g\circ\epsilon_D=\epsilon_{D'}\circ LR(g)$ so that the naturality condition is satisfied. \\

We now show that the triangle conditions are satisfied. We have that 
\begin{align*}
...
\begin{prp}{}{} Let $U:\bold{Top}\to\bold{Set}$ be the forgetful functor. Then $U$ admits a left adjoint and a right adjoint. 
\begin{proof}
...
\begin{prp}{}{} Let $U:\bold{Grp}\to\bold{Set}$ be the forgetful functor. Then $U$ admits a left adjoint. 
\begin{proof}
...
\begin{lmm}{}{} Let $F:\mC\to\mD$ be a functor with a left adjoint $L$. Then for every small category $\mJ$, the functor $$F\circ(-):\mC^\mJ\to\mD^\mJ$$ has a left adjoint by $L\circ(-)$. 
\begin{proof}
...
\end{itemize} 
\begin{proof}
Let $X:\mJ\to\mC$ be a diagram with limit $\Delta\lim_\mJ X\to X$. Let $L$ be the left adjoint of $F$. We want to find a natural isomorphism $$\Hom_\mD\left(-,F\left(\lim_\mJ X\right)\right)\cong\text{Nat}(\Delta(-),F\circ X)$$
...
\begin{lmm}{}{} Let $U:\mA\to\mS$ be a functor. Then $U$ admits a left adjoint if and only if for any $S\in\text{Obj}\mS$, there is a map $S\overset{i}{\to}U(A)$ such that for all $f:S\to U(B)$ in $\mS$, there is a unique map $\lambda:B\to A$ such that the following diagram commutes: \\
\adjustbox{scale=1.0,center}{\begin{tikzcd}
	S && {U(A)} \\
	\\
	&& {U(B)}
...
\begin{defn}{Weakly Initial Elements}{} Let $\mC$ be a category. An object $C\in\text{Obj}(\mC)$ is weakly initial if for every $D\in\text{Obj}(\mC)$, there is a map $C\to D$. A set of objects $\{C_i\;|\;i\in I\}$ is jointly weakly initial if for every $D\in\text{Obj}(\mC)$, there is a map $C_i\to D$ for some $i\in I$. 
\end{defn}
...
\begin{thm}{General Adjoint Functor Theorem}{} Let $\mA$ be a locally small and complete category. Let $U:\mA\to\mS$ be a functor which preserves limits. Suppose that $U$ satisfies the solution set condition: For any $S\in\text{Obj}\mS$, there is a set of maps $$\Psi_S=\{f_i:S\to U(A_i)\}$$ such that for every morphism $f:S\to U(B)$ there exists an $f_i\in\Psi_i$ and $\lambda:A_i\to B$ such that \\
\adjustbox{scale=1.0,center}{\begin{tikzcd}
	S && {U(A_i)} \\
	\\
	&& {U(B)}
...
First notice that since $U$ preserves limits, $S/U$ is complete: Given $X:\mJ\to S/U$, then $$X_J=\left(A_J\in\text{Obj}\mA,f_J:S\to U(A_J)\right)$$ and one can check that $$\lim_\mJ X=\left(\lim_{J\in\mJ}A_J,S\overset{\{f_J\}}{\to}\lim_{J\in\text{Obj}\mJ}U(A_J)\overset{\cong}{\to}U\left(\lim_JA_J\right)\right)$$ It is clear that $S/U$ has a set of jointly weakly initial objects $\Psi=\Psi_J$. \\
