...
\begin{eg}{}{} There is an isomorphism given by $$\frac{\Z[x]}{(x+1,x^2+2)}\cong\Z/3\Z$$ 
\begin{proof}
...
Let $R$ be a commutative ring. Recall that two ideals $I,J$ are coprime if $I+J=R$. In particular, this implies that $IJ=I\cap J$. Then the Chinese Remainder theorem reads as $$\frac{R}{\prod_{i=1}^kI_i}=\frac{R}{\bigcap_{i=1}^kI_i}\cong\prod_{i=1}^k\frac{R}{I_i}$$ More generally, if $I$ and $J$ are not coprime, then the relation between $R/(I\cap J)$ and $R/I\times R/J$ is given by the following exact sequence: 

\begin{lmm}{}{}\\
Let $R$ be a commutative ring. Let $I,J$ be ideals of $R$. Then there is a short exact sequence of the form \\
\adjustbox{scale=1.0,center}{\begin{tikzcd}
	0 & {\frac{R}{I\cap J}} && {\frac{R}{I}\oplus\frac{R}{J}} && {\frac{R}{I+J}} & 0
	\arrow[from=1-1, to=1-2]
	\arrow["{[a]\mapsto([a],[a])}", from=1-2, to=1-4]
	\arrow["{([a],[b])\mapsto[a-b]}", from=1-4, to=1-6]
...
\begin{prp}{}{} Let $R$ be a commutative ring. Then we have $$N(R)=\bigcap_{\substack{P\text{ is a prime}\\\text{ideal of }R}}P$$ 
\begin{proof}
...
\begin{eg}{}{} Consider the ring $$R=\frac{\C[x,y]}{(x^2-y,xy)}$$ Then its nilradical is given by $N(R)=(x,y)$. 
\begin{proof}
...
\begin{lmm}{}{} Let $R$ be a commutative ring. Then $x\in J(R)$ if and only if $1-xy\in R^\times$ for all $y\in R$. 
\begin{proof}
...
\item $J$ is a prime ideal if and only if $\varphi(J)=J/I$ is a prime ideal and so $\varphi$ induces an inclusion preserving bijection $$\text{Spec}(R/I)\;\;\overset{1:1}{\longleftrightarrow}\;\;\{P\in\text{Spec}(R)\;|\;I\subseteq P\}$$
\item $J$ is a maximal ideal if and only if $\varphi(J)=J/I$ is a maximal ideal and so $\varphi$ induces an inclusion preserving bijection $$\text{maxSpec}(R/I)\;\;\overset{1:1}{\longleftrightarrow}\;\;\{m\in\text{maxSpec}(R)\;|\;I\subseteq m\}$$
\end{itemize} 
\begin{proof}\\
\begin{itemize}
...
\subsection{Commutative Graded Rings}
Not to be confused with graded commutative rings, commutative graded rings are commutative rings that admit a grading. 

\begin{lmm}{}{}\\
Let $R$ be a commutative graded ring. Let $I$ be a homogeneous ideal of $R$. Then $\sqrt{I}$ is also homogeneous. 
\end{lmm}

Let $R=\bigoplus_{n=0}^\infty R_n$ be a commutative graded ring. Recall that the irrelevant ideal is defined to be $$R_+=\bigoplus_{n=1}^\infty R_n$$ Moreover, for a homogeneous ideal $I\subseteq R$, we defined $I_+=I\cap R_+$. 

\begin{lmm}{}{}\\
Let $R$ be a graded ring. Let $I\subseteq R$ be a homogeneous ideal. Then we have $$\sqrt{I}_+=\bigcap_{I\subseteq P\text{ is relevant }}P$$
\begin{proof}
We have seen that $I\subseteq P$, then $\sqrt{I}\subseteq P$. Now let $f\notin\sqrt{I}$. Define $$S=\{I\subset S_+\;|\;I\text{ is a homogeneous ideal such that }I\cap\{f^n\;|\;n\in\N\}=\emptyset\text{ and }I\subseteq P\}$$ (Apply Zorn's lemma then prove that the maximal element is prime, then $f$ is not contained int he intersection of all relevant ideals containing $I$). 
\end{proof}
\end{lmm}
...
\begin{crl}{}{} Let $R$ be an Artinian local ring. Then the following are true. 
\begin{itemize}
\item $N(R)$ is the unique maximal / prime / radical ideal of $R$. 
...
\begin{prp}{}{}\\
Let $R=\bigoplus_{n\in\N}R_i$ be a graded ring where $R_0$ is a commutative ring. Then the following are equivalent. 
\begin{itemize}
\item $R$ is Noetherian. 
\item $R_0$ is Noetherian and $R_+$ is finitely generated. 
\item $R_0$ is Noetherian and $R$ is a finitely generated $R_0$-algebra. 
\end{itemize}
\begin{proof}
\begin{itemize}
\item $(1)\implies(2)$: Suppose that $R$ is Noetherian. Then $R_0\cong R/R_+$ is a quotient of $R$ and so is Noetherian. Since $R_+$ is a homogeneous ideal of $R$, it is finitely generated. 
\item $(2)\implies(3)$: Suppose that $R_+$ is finitely generated by $x_1,\dots,x_r$ where each $x_i$ is of degree $d_i$. Then we have $$R_n=\sum_{i=1}^r x_1R_{n-{d_i}}$$ Indeed, the expression on the right lies in $R_n$. Conversely, given $x\in R_n$, there exists $f_1,\dots,f_r\in R$ such that $x=\sum_{i=1}^rx_if_i$. If $g_i$ is the homogeneous term of $f_i$ of degree $d_i$, then we have $x=\sum_{i=1}^rx_ig_i$. Hence $R_n$ is a subset of the expression on the right. This implies that $R=R_0[x_1,\dots,x_r]$ and so $R$ is a finitely generated $R_0$-algebra. 
\item $(3)\implies(1)$: By Hilbert's basis theorem we know that $R$ is Noetherian. 
\end{itemize}
\end{proof}
\end{prp}
...
This is a key property of local rings. Let $R$ be a commutative ring. Let $M$ be an $R$-module. When $M$ is finitely generated, it makes sense to ask for a minimal set of generators for $M$ as an $R$-module (A generating set $S$ is minimal if $S\setminus\{x\}$ does not generate the module for all $x\in S$). In general, this number is not well defined. For example, consider $R=\frac{k[x,y]}{(x^2,xy,y^2,x+y-1)}$ for $k$ a field and $R$ as a module over itself. Then $\{1\}$ and $\{x,y\}$ are both minimal generating sets. \\

However, the corollaries for Nakayama's lemma show that the cardinality for any minmal set of generators is well behaved since it is governed by the dimension of $M/mM$ as a $R/m$-vector space. 
...
\begin{prp}{}{} Let $A$ be a commutative algebra over a ring $R$. Then the following are equivalent. 
\begin{itemize}
...
\begin{prp}{}{}\\
Let $k$ be a field. Let $A,B$ be $k$-algebras. Let $\varphi:A\to B$ be a $k$-algebra homomorphism. Let $m$ be a maximal ideal of $B$. If $B$ is finitely generated over $k$, then $\varphi^{-1}(m)$ is a maximal ideal of $A$. 
\end{prp}
...
\begin{eg}{}{}\\
Let $k$ be a field. Then $R=\frac{k[x,y,z,w]}{(yw-z^2,xw-yz,xz-y^2)}$ is a finitely generated free $k[x,w]$-module. 
\begin{proof}\\
Let $A=k[x,w]\cdot 1\oplus k[x,w]\cdot y\oplus k[x,w]\cdot z$ be the $k[x,w]$-module with multiplication defined by $y^2=xz$, $yz=xw$ and $z^2=yw$. It is easy to check that this defines a $k[x,w]$-algebra. Define the map $k[x,w,y,z]\to A$ by sending $x$ to $x$, $y$ to $y$, $z$ to $z$ and $w$ to $w$. This descends to a well defined $k$-algebra homomorphism $\psi:R\to A$ because $\psi(y^2-xz)=\psi(yz-xw)=\psi(z^2-yw)=0$. This is map is a left inverse to the map $\varphi:A\hookrightarrow k[x,y,z,w]\to R$ and so $\varphi$ is injective. Moreover, $\varphi$ is clearly surjective since $R$ is generated as an $k[x,w]$-module by $1,y,z$. 
\end{proof}
\end{eg}
...
So localization really is just a tensor product with the localization of a ring. Indeed, the idea of localization of a ring is to allow certain elements of a ring to be invertible, and the localization of a module is to allow these inverted elements to also act on it. 
...
\begin{defn}{The Total Quotient Ring}{}\\
Let $R$ be a commutative ring. Define the total quotient ring of $R$ by $$Q(R)=\{r\in R\;|\;r\text{ is not a zero divisor }\}^{-1}R$$
\end{defn}
...
\begin{eg}{}{}\\
Let $R$ be a commutative ring. Let $d\in R$ be squarefree. Then we have $$\text{Frac}\left(\frac{R[x]}{(x^2-d)}\right)=\frac{\text{Frac}(R)[x]}{(x^2-d)}$$
\begin{proof}\\
Notice that the quotient on the right is a field since $x^2-d$ is irreducible. Since it also contains $\frac{R[x]}{(x^2-d)}$ and $\text{Frac}\left(\frac{R[x]}{(x^2-d)}\right)$ is the smallest field containing $\frac{R[x]}{(x^2-d)}$, we must have that $\text{Frac}\left(\frac{R[x]}{(x^2-d)}\right)\subseteq\frac{\text{Frac}(R)[x]}{(x^2-d)}$. On the other hand, notice that any element $(r_0/s_0)+(r_1/s_1)x\in\frac{\text{Frac}(R)[x]}{(x^2-d)}$ is contained in $\text{Frac}\left(\frac{R[x]}{(x^2-d)}\right)$ since $(r_0/s_0)+(r_1/s_1)x=\frac{s_1r_0+s_0r_1x}{s_0s_1}$, hence both fields contain each other. 
\end{proof}
\end{eg}
...
\begin{prp}{}{}\\
Let $R$ be a commutative $\N$-graded ring. Let $S$ be a multiplicative subset of $R$ consisting only of homogeneous elements. Then $$S^{-1}R$$ is also a graded ring with grading defined by $\deg(a/f^k)=\deg(a)-k$. 
\end{prp}

\begin{prp}{}{}\\
Let $R$ be a commutative $\N$-graded ring. Let $f\in R_+$. Then there is a bijection $$\{P\in\text{Spec}(R_f)\;|\;P\text{ is homogeneous }\}\;\;\overset{1:1}{\longleftrightarrow}\;\;\text{Spec}((R_f)_0)$$ induced by the ring homomorphism $(R_f)_0\hookrightarrow R_f$. 
\begin{proof}\\
The above homomorphism defines the map from left to right by $P\mapsto\varphi^{-1}(P)$ where $\varphi:(R_f)_0\hookrightarrow R_f$ is the inclusion map. This map is clearly is well defined. For the reverse direction, let $P\in\text{Spec}((R_f)_0)$. Define $f:\text{Spec}((R_f)_0)\to\{P\in\text{Spec}(R_f)\;|\;P\text{ is homogeneous }\}$ by $f(P)=\bigoplus_{k\in\N}Q_k$ where $Q_k\subseteq R_k$ and $a\in Q_k$ if and only if $a^{\deg(f)}/f^k\in P$. 
\end{proof}
\end{prp}

\begin{defn}{Localization of a Graded Ring}{}\\
Let $R$ be a commutative graded ring. Let $P$ be a homogeneous prime ideal of $R$. Let $T=\{r\in R\setminus P\;|\;r\text{ is homogeneous }\}$. Define the localization of $R$ at $P$ as a graded ring to be $$R_{(P)}=T^{-1}R$$
\end{defn}

\begin{prp}{}{}\\
Let $R$ be a commutative graded ring. Let $P$ be a homogeneous prime ideal of $R$. Then the localization $R_P$ as a graded ring is also a graded ring with grading defined by $\deg(f/g)=\deg(f)-\deg(g)$ for $f/g\in R_P$. 
\end{prp}
...
\begin{proof}\\
Let $r/s\in S^{-1}\overline{A}$. Then there exists $a_0,\dots,a_{n-1}\in A$ such that $r^n+\sum_{i=0}^{n-1}a_ir^i=0$. Dividing by $s^n$ gives $$(r/s)^n+\sum_{i=0}^{n-1}(a_i/s^{n-i})(r/s)^i=0$$ Hence $r/s\in\overline{S^{-1}A}$. \\

Conversely, suppose that $r/s\in\overline{S^{-1}A}$. Then there exists $f_i/g_i\in S^{-1}A$ such that $$(r/s)^n+\sum_{i=0}^{n-1}(f_i/g_i)(r/s)^i=0$$ Multiplying by $(sg_0\cdots g_{n-1})^n$ gives $$(g_0\cdots g_{n-1}r)^n+\sum_{i=0}^{n-1}(f_is^{n-i}g_0^{n-i}\cdot g_{i-1}^{n-i}\cdot g_i^{n-i-1}\cdot g_{i+1}^{n-i}\cdots g_{n-1}^{n-i})(g_0\cdots g_{n-1}\cdot r)^i=0$$ Hence $g_0\cdots g_{n-1}\cdot r\in\overline{A}$. Since each $g_i\in S$, we have that $r/s=(g_0\cdots g_{n-1}\cdot r)/(g_0\cdots g_{n-1}\cdot s)\in S^{-1}\overline{A}$. 
\end{proof}
\end{prp}
...
\subsection{Four Main Theorems of Integral Extensions}
...
\begin{thm}{The Lying Over Theorem}{}\\
Let $A,B$ be rings such that $A\subset B$ is an integral extension. Let $P$ be a prime ideal of $A$. Then there exists a prime ideal $Q$ of $B$ such that $P=Q\cap A$
\begin{proof}\\
...
\begin{thm}{The Incomparability Theorem}{} Let $A,B$ be rings such that $A\subset B$ is an integral extension. If $Q_1\neq Q_2$ are distinct prime ideals of $B$ such that $Q_1\cap A=P=Q_2\cap A$, then $Q_1\not\subset Q_2$ and $Q_2\not\subset Q_1$. 
\begin{proof}\\
We prove the contrapositive statement. That is, if $Q_1,Q_2$ are prime ideals of $B$ such that $Q_1\cap A=P=Q_2\cap B$ and $Q_1\subseteq Q_2$, then $Q_1=Q_2$. \\
...
\begin{thm}{The Going-Up Theorem}{} Let $A,B$ be commutative rings such that $A\subset B$ is an integral extension. Let $0\leq m<n$. Consider the following situation \\
\adjustbox{scale=1.0,center}{\begin{tikzcd}
...
\end{itemize} 
Moreover, if the chain of prime ideals of $A$ is strictly increasing the new chain of prime ideals in $B$ is strictly increasing. 
\begin{proof}
By induction, it suffices to prove the case $m=1$ and $n=2$. This means that we want to find a prime ideal $Q_2$ such that $Q_1\subseteq Q_2$ and $Q_2\cap A=P_2$. By 7.2.6, $B/Q_1$ is integral over $A/P_1$. Since $P_2/P_1$ is a prime in $A/P_1$ by the correspondence theorem, by 7.3.2 there exists a prime ideal $Q_2/Q_1$ in $B/Q_1$ such that $Q_2/Q_1\cap A/P_1=P_2/P_1$. This implies that $Q_2\cap A=P_2$. \\

Finally, suppose that the chain of prime ideals in $A$ is strictly increasing. Suppose for a contradiction that $Q_i=Q_{i+1}$. Then we must have $P_i=Q_i\cap A=Q_{i+1}\cap A=P_{i+1}$, a contradiction. 
\end{proof}
\end{thm}
...
\begin{eg}{}{}\\
Let $R$ be a UFD such that $2\in R$ is invertible. Let $d\in R$ be such that $x^2-d\in R[x]$ is irreducible. If $d$ is squarefree, then $\frac{R[x]}{(x^2-r)}$ is normal. 
\begin{proof}\\
Suppose that $d\in R$ is squarefree. By 5.3.5 we have that $$\text{Frac}\left(\frac{R[x]}{(x^2-d)}\right)=\frac{\text{Frac}(R)[x]}{(x^2-d)}$$ so that the fraction field of $\frac{R[x]}{(x^2-d)}$ is an algebraic extension of $\text{Frac}(R)$ of degree $2$. Let $\alpha\in\overline{\frac{R[x]}{(x^2-d)}}$. Then there exists a monic polynomial $f\in\frac{R[x]}{(x^2-d)}[y]$ such that $f(\alpha)=0$. Now notice that the field extension of $\text{Frac}\left(\frac{R[x]}{(x^2-d)}\right)$ over $\text{Frac}(R)$ has degree $2$, and is a galois extension since it splits the separable polynomial $x^2-d$. Then the Galois group is given by $\{\text{id},\sigma\}$. Now since the norm of a Galois extension is given by the product of elements of the Galois group, we have $$N(f)=f\cdot\sigma(f)\in\text{Frac}(R)[y]$$ Since $f\in\frac{R[x]}{(x^2-d)}[y]$, we must have that $N(f)\in R[y]$. Moreover, $N(f)$ is clearly monic and $N(f)(\alpha)=0$. \\

On the other hand, $\alpha$ has the form $a_0+a_1x$ for $a_0,a_1\in\text{Frac}(R)$. Notice that the minimal polynomial of $\alpha$ is $$p(y)=y^2-2a_0y+a_0^2-a_1^2d\in\text{Frac}(R)[y]$$ Indeed, no linear polynomial can be a minimal polynomial for $\alpha$. Then by definition of the minimal polynomial, there is a factorization $N(f)=p(y)q(y)$ for some $q\in\text{Frac}(R)[y]$. Since $R$ is normal, we can apply 7.2.9 Gauss's lemma to see that the factorization $N(f)=p(y)q(y)$ exists in $R[y]$. Hence $p(y)\in R[y]$, and so $2a_0,a_0^2-a_1^2d\in R$. \\

Since $2$ is invertible in $R$, we must have $a_0\in R$. Since $a_0\in R$ and $a_0^2-a_1^2d\in R$, we must have $a_1^2d\in R$. Now since $a_1\in\text{Frac}(R)$, suppose that $a_1=r/s$ for $r,s\in R$ and $s\neq 0$ and $r,s$ in lowest terms (possible since $R$ is a UFD). Then $\frac{r^2d}{s^2}\in R$ implies that $s$ is invertible in $R$. Hence $a_1\in R$. Thus $\alpha=a_0+a_1x\in\frac{R[x]}{(x^2-d)}$ so that $\frac{R[x]}{(x^2-d)}$ is normal. 
\end{proof}
\end{eg}

\begin{eg}{}{}\\
The following rings are normal. 
\begin{itemize}
\item $\frac{\Z[x]}{(x^2-n)}$ where $n$ is squarefree and $n\equiv2,3\;(\bmod\;4)$. 
\item $\Z\left[\frac{1+\sqrt{n}}{2}\right]$ where $n$ is squarefree and $n\equiv1\;(\bmod\;4)$. 
\end{itemize}
\begin{proof}\\
Let $\alpha=a+bx\in\overline{\frac{\Z[x]}{(x^2-n)}}\subseteq\Q(\sqrt{n})$. The proof for the first point mimics the above until we reach that $2a\in\Z$ and $a^2-b^2n\in\Z$. Then we have $(2a)^2-(2b)^2n=4(a^2-b^2n)\in\Z$. Then following the above example, we must have that $2b\in\Z$. Now we have that $$(2a)^2-(2b)^2n\equiv 0\;(\bmod\;4)$$ which gives $(2a)^2\equiv(2b^2)n\;(\bmod\;4)$. Now recall that for any $x\in\Z$, we must have $x^2\equiv0,1(\bmod\;4)$. \\

If $n\equiv2\;(\bmod\;2)$, then $(2a)^2\equiv(2b)^2\equiv0(\bmod\;4)$. In this case, $2a$ and $2b$ are even numbers and so $a,b\in\Z$.  If $n\equiv3\;(\bmod\;4)$, then again $(2a)^2\equiv(2b)^2\equiv0\;(\bmod\;4)$ so that $a,b\in\Z$. Hence $\alpha\in\frac{Z[x]}{(x^2-n)}$ so that the ring is integral.\\

Let $\alpha=a+b\frac{1+\sqrt{n}}{2}\in\overline{\Z\left[\frac{1+\sqrt{n}}{2}\right]}\subseteq\Q\left(\frac{1+\sqrt{5}}{2}\right)$. Notice that the minimal polynomial of $\alpha$ in $\Q$ is given by $x^2-(2a+b)x+\frac{(2a+b)^2-nb^2}{4}$. Using a similar method as above, we find that $2a+b\in\Z$ and $\frac{(2a+b)^2-nb^2}{4}\in\Z$. This means that $(2a+b)^2-nb^2\in 4\Z$. This gives $b\in\Z$. Now since $n\equiv1\;(\bmod\;4)$, we must have $(2a+b)^2\equiv b^2\equiv 0,1\;(\bmod\;4)$. If $(2a+b)^2$ and $b^2$ are divisible by $4$, then $2a+b$ and $b$ is even. Hence $a\in\Z$. If $(2a+b)^2$ and $b^2$ are divisible by $1$, then $2a+b$ and $b$ are both odd so that $2a$ is even. Hence $a\in\Z$. Thus $\alpha\in\Z\left[\frac{1+\sqrt{n}}{2}\right]$ so that $\Z\left[\frac{1+\sqrt{n}}{2}\right]$ is normal. 
\end{proof}
\end{eg}

\begin{eg}{}{}\\
Let $k$ be an algebraically closed field. The following rings are normal. 
\begin{itemize}
\item $\frac{k[x_1,\dots,x_n]}{(x_1^2+\dots+x_m^2)}$ where $n\geq m\geq 3$. 
\item $\frac{k[w,x,y,z]}{(wz-xy)}$. 
\end{itemize}
\begin{proof}\\
I first claim that $x_1^2+\dots+x_k^2$ for $2\leq k\leq n$ is irreducible. We proceed by induction. Consider the base case $k=2$. Consider $x_1^2+x_2^2$ as a polynomial in $x_1$ over $k[x_2,\dots,x_n]$. Clearly $(x_2-1)$ is a prime ideal since $k[x_2,\dots,x_n]/(x_2-1)\cong k$ or $k[x_3,\dots,x_n]$ depending on $n\geq 3$ or not. In both cases they are integral domains. Then clearly $(x_2-1)$ satisfies the conditions in Eisenstein's criterion so that $x_1^2+x_2^2$ is irreducible in $k[x_1,\dots,x_n]$. Now suppose that $x_1^2+\dots+x_k^2$ is irreducible in $k[x_1,\dots,x_n]$ for $k\leq n-1$. I claim that $x_1^2+\dots+x_{k+1}^2$ is irreducible. We consider it as a polynomial over $x_1$ in $k[x_2,\dots,x_n]$ By assumption, $x_2^2+\dots+x_{k+1}^2$ is irreducible. Since $k[x_2,\dots,x_n]$ is a UFD, $(x_2^2+\dots+x_{k+1}^2)$ is a prime ideal whose square does not contain $x_2^2+\dots+x_{k+1}^2$. Hence by Eisenstein's criterion $x_1^2+\dots+x_{k+1}^2$ is irreducible. \\

Now let $m\geq 3$. Then $-(x_2^2+\dots+x_m^2)$ is irreducible and hence squarefree. Then by 7.5.8 we conclude that $\frac{k[x_1,\dots,x_n]}{(x_1^2+\dots+x_m^2)}$ is normal. \\

For the second point, notice that there is a $k$-algebra isomorphism $$k[w',x,y,z']\cong k[w,x,y,z]$$ where $w=w'+z'$ and $z=w'-z'$ (possible since $2\neq 0$ in $k$). This induces an isomorphism $$\frac{k[w,x,y,z]}{(wz-xy)}\cong\frac{k[w',x,y,z']}{((w')^2-((z')^2+xy))}$$ Rewriting coordinates, we may deal with the ring $\frac{k[w,x,y,z]}{w^2-(z^2+xy)}$. I claim that $z^2+xy$ is irreducible. To prove this, consider $z^2+xy$ as a polynomial in $z$ over $k[x,y]$. Then notice that $(x)$ is a prime ideal in $k[x,y]$ such that $xy\notin(x^2)$. Hence by Eisenstein's criterion, we have that $z^2+xy$ is irreducible, and hence squarefree. Then we may apply 7.5.8 to conclude that our given ring is normal. 
\end{proof}
\end{eg}
...
\begin{lmm}{}{}\\
Let $R,S$ be commutative rings such that $R\subseteq S$ is an integral extension. Then we have $$\dim(R)=\dim(S)$$
\begin{proof}
Let $P_0\subset\cdots\subset P_n$ be a strictly increasing chain of prime ideals of $R$. By the going up theorem, we obtain an increasing chain of prime ideals $Q_0\subseteq\cdots\subseteq Q_n$ in $S$. Hence $\dim(R)\leq\dim(S)$. \\

Conversely, suppose that $Q_0\subset\cdots\subset Q_n$ is a strictly increasing chain of prime ideals in $S$. Then $\varphi^{-1}(Q_0)\subseteq\cdots\subseteq\varphi^{-1}(Q_n)$ is an increasing chain of prime ideals of $R$. Hence $\dim(S)\leq\dim(R)$. 
\end{proof}
\end{lmm}
...
\begin{lmm}{}{}\\
Let $k$ be a field. Let $T$ be a finite set of monomials in $k[x_1,\dots,x_n]$. Then there exists $w_1,\dots,w_{n-1}$ such that for any two $a_{k_1,\dots,k_n}x_1^{k_1}\cdots x_n^{k_n},b_{r_1,\dots,r_n}x_1^{r_1}\cdots x_n^{r_n},\in T$, $(k_1,\dots,k_n)\neq(j_1,\dots,j_n)$ implies that $$\sum_{i=1}^{n-1}w_ik_i+k_n\neq\sum_{i=1}^{n-1}w_ij_i+j_n$$
\begin{proof}
We induct on $n$. When $n=1$ then we are done. When $n\geq 2$, we can write each monomial in $T$ as a monomial in $x_2,\dots,x_n$ to apply the inductive hypothesis and obtain $w_2,\dots,w_n$. Then set $w_1>\max\{\sum_{i=2}^{n-1}w_ik_i+k_n\;|\;(k_1,\dots,k_n)\text{represents a monomial in }T\}$. For $(k_1,\dots,k_n)\neq(j_1,\dots,j_n)$ there are two cases. If $(k_2,\dots,k_n)\neq(j_2,\dots,j_n)$, then by inductive hypothesis we have $\sum_{i=2}^{n-1}w_ik_i+k_{n-1}\neq\sum_{i=2}^{n-1}w_ij_i+j_n$. Since $w_1$ is larger than these two numbers, we must have that $\sum_{i=1}^{n-1}w_ik_i+k_n\neq\sum_{i=1}^{n-1}w_ij_i+j_n$. Otherwise, if $(k_2,\dots,k_n)=(j_2,\dots,j_n)$ but $k_1\neq j_1$, then $w_1k_1\neq w_1j_1$ and so we are done. 
\end{proof}
\end{lmm}

\begin{thm}{Noether Normalization Lemma}{}\\
Let $k$ be a field. Let $R$ be a finitely generated $k$-algebra with the isomorphism $R\cong\frac{k[x_1,\dots,x_n]}{I}$ for some ideal $I$. Then there exists $y_1,\dots,y_d\in R$ for some $0\leq d\leq n$ such that the following are true. 
\begin{itemize}
\item $y_1,\dots,y_d$ are algebraically independent. 
\item $R$ is finitely generated over $k[y_1,\dots,y_d]$. 
\item $d=\dim(R)$. 
\end{itemize}
\begin{proof}\\
Since $R$ is a finitely generated $k$-algebra, we have $R\cong\frac{k[x_1,\dots,x_n]}{I}$ for some $n\in\N$ and some ideal $I$. We induct on $n$. When $n=1$, we have $R\cong\frac{k[x]}{I}$. Since $k[x]$ is a PID, $I=(f)$ for some $f\in k[x]$. If $f=0$, then we are done. So suppose that $f\neq 0$. Then $R$ is a generated as a $k$-vector space by $1,x,\dots,x^{\deg(f)-1}$. Thus $R$ is finitely generated over $k[x]$. \\

Now suppose that $n\geq 2$. If each $x_1,\dots,x_n$ are algebraically independent then we are done. So suppose not. Then I claim that there is a $k$-subalgebra $S$ of $R$ generated by $n-1$ elements such that $R$ is finitely generated over $S$. Since $x_1,\dots,x_n$ are not algebraically independent, there exists a polynomial in $k$ such that $f(x_1,\dots,x_n)=0$. For $r_1,\dots,r_{n-1}\in\N$, write $z_i=x_i-x_n^{r_i}$. Then we have $$f(z_1+x_n^{r_1},\dots,z_{n-1}+x_n^{r_{n-1}},x_n)=0$$ If the polynomial $f$ has coefficients $a_{k_1,\dots,k_n}$, then expanding out the expression, we have $$\sum a_{k_1,\dots,k_n}\prod_{i=1}^{n-1}(z_i+x_n^{r_i})^{k_i}x_n^{k_n}=0$$ Each term in the summand has a unique term with highest order of $x_n$ after expanding, which is given by $a_{k_1,\dots,k_n}x_n^{\sum_{i=1}^nr_{i-1}k_i+k_n}$. By the above lemma, we can find $r_1,\dots,r_{n-1}$ such that $(k_1,\dots,k_n)\neq(j_1,\dots,j_n)$ implies that $\sum_{i=1}^nr_{i-1}k_i+k_n\neq\sum_{i=1}^nr_{i-1}j_i+j_n$. Then this means that there is a unique term that achieves the maximum of $\sum_{i=1}^{n-1}r_ik_i+k_n$. This proves that the highest term of $f$ as a polynomial in $y_n$ has coefficient $a_{k_1,\dots,k_n}$ for some $k_1,\dots,k_n$. Dividing out $f$ by this coefficient, we deduce that $x_n$ is an integral element over $k[z_1,\dots,z_{n-1}]$. This proves that $S=k[z_1,\dots,z_{n-1}]$ is a $k$-subalgebra of $R$ such that $R$ is finitely generated over $S$. \\

By the inductive hypothesis, there exists $y_1,\dots,y_d$ for some $0\leq d\leq n-1$ such that $y_1,\dots,y_d$ are algebraically independent and $S$ is finitely generated over $k[y_1,\dots,y_d]$. Since $R$ is finitely generated over $S$ and $S$ is finitely generated over $k[y_1,\dots,y_d]$, we conclude that $R$ is finitely generated over $k[y_1,\dots,y_d]$. \\

It remains to show that $d=\dim(R)$. Since $R$ is integral over $S=k[y_1,\dots,y_d]$, we have $\dim(R)=\dim(S)$ and so it suffices to show that $d=\dim(S)$. We induct on $d$. When $d=0$, then $S=k$ and so $\dim(S)=0$. Now let $d>0$. Then $0\subset(y_1)\subset(y_1,y_2)\subset\cdots\subset(y_1,\dots,y_d)$ is a chain of prime ideals in $S$ so that $d\leq\dim(S)$. Conversely, suppose that $0\subset P_1\subset\cdots\subset P_n$ is a strictly increasing chain of prime ideals in $S$. Let $0\neq u\in P_1$. By the Noether normalization lemma, because first variable is always trivially algebraically independent, there exists $t_2,\dots,t_d\in S$ such that $S$ is integral over $T=k[u,t_2,\dots,t_d]$. By the inductive hypothesis, we have $d-1=\dim(T/(u))$. By the going up theorem, $0\subset P_1\cap T\subset\cdots\subset P_n\cap T$ is a strictly increasing chain of prime ideals in $T$. Then in $T/(P_1\cap T)$ the chain becomes a strictly increasing chain of prime ideals of length $n-1$. Since $\dim(T/(P_1\cap T)\leq\dim(T/(u))$, we have that $n-1\leq d-1$ and hence $\dim(S)\leq d$. 
\end{proof}
\end{thm}

\begin{prp}{}{}\\
Let $k$ be a field. Let $R$ be a finitely generated $k$-algebra that is an integral domain. Then we have $$\dim(R)=\text{trdeg}_k(\text{Frac}(R))$$
\begin{proof}
By the Noether normalization lemma, there exists algebraically independent elements $y_1,\dots,y_{\dim(R)}$ such that $R$ is finitely generated over $k[y_1,\dots,y_{\dim(R)}]$. Then $\text{Frac}(R)$ is algebraic over $\text{Frac}(k[y_1,\dots,y_{\dim(R)}])=k(y_1,\dots,y_{\dim(R)})$. Hence $\text{trdeg}_k(\text{Frac}(R))=\dim(R)$. 
\end{proof}
\end{prp}

\begin{crl}{}{}\\
Let $k$ be a field. Then we have $$\dim(k[x_1,\dots,x_n])=n$$
\begin{proof}
The transcendence degree of $\text{Frac}(k[x_1,\dots,x_n])=k(x_1,\dots,x_n)$ is $n$. 
\end{proof}
\end{crl}
...
\begin{prp}{}{} Let $k$ be a field. Let $R$ be a finitely generated $k$-algebra that is an integral domain. Then we have $$\dim(A)=\dim(A/P)+\text{ht}_A(P)$$
\end{prp}
...
\adjustbox{scale=1.0,center}{\begin{tikzcd}
	0 & {m^iM} & {m^{i-1}M} & {\frac{m^{i-1}M}{m^iM}} & 0
	\arrow[from=1-1, to=1-2]
	\arrow[from=1-2, to=1-3]
	\arrow[from=1-3, to=1-4]
	\arrow[from=1-4, to=1-5]
