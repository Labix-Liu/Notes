...
Let $R,S$ be commutative rings. Let $\varphi:R\to S$ be a ring homomorphism. Let $M$ be an $S$-module. Define the restriction of $M$ to $R$ to be the abelian group $M$ together with module action $$r\cdot_R m=\varphi(r)\cdot_S m$$ for all $r\in R$. We denote the new $R$-module as $M|_R$. 
\end{defn}

\begin{prp}{}{} Let $R,S$ be commutative rings. Let $\varphi:R\to S$ be a ring homomorphism. Then there is an isomorphism $$\Hom_S(S\otimes_R M,N)\cong\Hom_R(M,N)$$ for any $R$-module $M$ and $S$-module $N$ given as follows. 
\begin{itemize}
