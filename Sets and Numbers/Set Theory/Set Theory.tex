...
\begin{prp}{}{} For every natural number $m\in\N$ there exists a function $s_m:\N\to\N$ such that
\begin{itemize}
\item $s_m(0)=m$
\item $s_m(n^+)=(s_m(n))^+$
\end{itemize}
...
\begin{prp}{Properties of Addition}{} Let $x,y,z\in\N$. 
\begin{itemize} 
...
\begin{prp}{}{} For every natural number $m\in\N$ there exists a function $p_m:\N\to\N$ such that 
\begin{itemize}
\item $p_m(0)=0$
\item $p_m(n^+)=p_m(n)+m$
\end{itemize}
...
Define multiplication in $\N$ to be the function $\cdot:\N\times\N\to\N$ by $$m\cdot n=p_m(n)$$
\end{defn}

\begin{prp}{}{} Let $x,y,z\in\N$. Then addition distributes through multiplication: $$x\cdot(y+z)=x\cdot y+x\cdot z$$
\begin{proof}\\
