...
\pagebreak
\section{The Homotopy Excision Theorem}
...
\subsection{Freudenthal Suspension Theorem}
\begin{thm}{Freudenthal Suspension Theorem}{}\\
Let $X$ be an $n$-connected CW complex. Then for $0\leq k\leq 2n$, the induced map $$\Sigma_\ast:\pi_k(X)\to\pi_{k+1}(\Sigma X)$$ is an isomorphism. For $k=2n+1$, $\Sigma_\ast$ is a surjection. 
\end{thm}
...
\pagebreak
\section{Loop Spaces and The Compact Open Topology}
...
Let $X,Y,Z$ be sets. Recall that in Category Theory, there is an adjunction $-\times Y\dashv\Hom_{\bold{Set}}(Y,-)$. This means that there is a natural bijection of sets $$\Hom_{\bold{Set}}(X\times Y,Z)\cong\Hom_{\bold{Set}}(X,\Hom_{\bold{Set}}(Y,Z))$$ When we equip the set of homomorphisms with the compact open topology, in general we lose the adjunction. 

\begin{prp}{}{}\\
Let $X,Y,Z$ be spaces. Let $f:X\times Y\to Z$ be a function. If $X$ is locally compact, then $f$ is continuous if and only if the map $Y\to\text{Map}(X,Z)$ given by $y\mapsto f(-,y)$ is continuous. 
\end{prp}
