...
\fancyhf{}
\rhead{Labix}
\lhead{Singular Homology and Cohomology}
\rfoot{\thepage}

\title{Singular Homology and Cohomology}
...
\begin{thm}{Universal Coefficient Theorem}{} Let $(X,A)$ be a pair of space. Let $T$ be an abelian group. Then there exists a natural map $h:H_n(X,A)\otimes T\to H_n(X,A;T)$ such that $\text{coker}(h)\cong\text{Tor}_1^\Z(H_{n-1}(X,A),T)$ and a split exact sequence (that is not natural) of the form \\~\\
\adjustbox{scale=1.0,center}{\begin{tikzcd}
