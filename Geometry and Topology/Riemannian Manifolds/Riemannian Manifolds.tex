...
\begin{prp}{}{}\\
Let $(M,g)$ be a Riemannian manifold. Let $p\in M$. Let $(U,x^1,\dots,x^n)$ be a chart at $p$. Then $g_p$ as an element of $T_p(M)^\ast\otimes T_p(M)^\ast$ is given locally on the chart by $$g_p=\sum_{i,j=1}^ng_{i,j}|_p\;(dx^i)|_p\otimes(dx^j)_p$$ where $g_{i,j}|_p=g_p\left(\frac{\partial}{\partial x^i}\bigg{|}_p,\frac{\partial}{\partial x^j}\bigg{|}_p\right)$ where the latter $g_p$ is thought of as an element of $(T_p(M)\otimes T_p(M))^\ast$. 
\end{prp}
...
Let $(M,g)$ be an orientable Riemannian manifold. Define the volume form $dV$ of $M$ to be given locally at $p\in M$ on the chart $(U,x^1,\dots,x^n)$ by $$dV_p=\sqrt{\abs{g_p}}dx^1|_p\wedge\cdots\wedge dx^n|_p$$
\end{defn}
...
\begin{prp}{}{}\\
Let $(M,g)$ be an oriented smooth manifold of dimension $3$. Then the following diagram is commutative: \\
