...
Let $k$ be a field. Let $X$ be an variety over $k$. We say that $X$ is an algebraic curve if $\dim(X)=1$. 
\end{defn}

\begin{eg}{}{}\\
Let $k$ be a field. Let $f\in k[x,y]$ be irreducible. Then $$\text{Spec}\left(\frac{k[x,y]}{(f)}\right)$$ is an irreducible curve over $k$. 
\end{eg}

\begin{prp}{}{}\\
Let $k$ be a field. Let $X$ be a curve over $k$. Let $p\in X$ be a closed point. Then the following are equivalent. 
\begin{itemize}
\item $\mO_{X,p}$ is a regular local ring. 
\item $\mO_{X,p}$ is a DVR. 
\item $\mO_{X,p}$ is normal. 
\end{itemize}
\end{prp}

Thus a curve is normal if and only if it is regular. Moreover, if $k$ is algebraically closed, then a curve is smooth if and only if it is regular. 
...
\begin{prp}{}{}\\
Let $k$ be an algebraically closed field. Let $C$ be an irreducible curve over $k$. Then $C$ is birational to a unique (up to isomorphism) smooth projective irreducible curve. 
\begin{proof}\\
...
\begin{prp}{}{}\\
Let $k$ be an algebraically closed field. Let $C$ be an irreducible curve over $k$. Then $C$ is projective if and only if $C$ is proper. 
\end{prp}

\begin{prp}{}{}\\
Let $k$ be an algebraically closed field. Let $C$ be an irreducible curve over $k$. Suppose that $C$ is birational to $\Prj^1$ (rational) and $C$ is not isomorphic to $\Prj^1$. Then the following are true. 
\begin{itemize}
...
\subsection{The Smooth Projective Irreducible Models}
\begin{prp}{}{}\\
Let $k$ be an algebraically closed field. Then there is an equivalence of categories between the following two categories. 
\begin{itemize}
\item The category of smooth projective irreducible curves over $k$. together with dominant rational maps. 
\item Finitely generated field extensions of $k$ of transcendence degree $1$. 
\end{itemize}
The equivalence is given by sending an irreducible curve $C$ to its function field $k(C)$. 
\end{prp}
...
\subsection{The Theory of Divisors}
Let $k$ be an algebraically closed field. Let $C$ be a smooth projective irreducible curve. Then we know that there are isomorphisms $$\text{Div}(C)\cong\text{CDiv}(C)\;\;\;\text{ and }\;\;\;\text{Cl}(C)\cong\text{CCl}(C)$$ 

Recall that the canonical divisor is a divisor $K_C$ such that $$\mL(K_C)=\Omega_{C/\C}^1$$ We may compute the divisor in the following way. Recall that given a line bundle $\mL$, to recover the divisor, we take the tensor product $\mL\otimes_{\mO_X}Q_X$. Locally at a chart we have $\mL(U)\otimes_{\mO_X(U)}Q_X(U)\cong Q_X(U)$. Now pick a global section $s$ of the tensor product. Locally at each $U_i$, it corresponds to a local section $f_i\in Q_X(U)$. Then $\{(U_i,f_i)\;|\;i\in I\}$ is precisely the Cartier data of the canonical divisor. \\

Now since $\Gamma(X,\Omega_{C/\C}^1\otimes_{\mO_X}Q_X)=\Omega_{\C(C)/\C}^1$, a choice of canonical divisor just corresponds to an non-zero element of $\Omega_{\C(C)/\C}^1$. Let us look at an example. 

\begin{eg}{}{}\\
Let $C=\V^H(x^2+y^2-z^2)\subseteq\Prj_\C^2$. Let $\omega=\frac{x+z}{z}d\left(\frac{x}{z}\right)\in\Omega_{\C(C)/\C}^1$ be a one-form. Then we have $$K_C=[1:0:1]+3[-1:0:1]-3[i:1:0]-3[-i:1:0]$$
\begin{proof}\\
...
Putting all the non-zero coefficients together with the corresponding points, we deduce that $$K_C=[1:0:1]+3[-1:0:1]-3[i:1:0]-3[-i:1:0]$$
\end{proof}
\end{eg}

\subsection{The Riemann-Roch Theorem}
\begin{defn}{Dimension of Global Sections}{}\\
Let $k$ be an algebraically closed field. Let $C$ be a smooth irreducible projective curve over $\C$. Let $D$ be a divisor on $C$. Define $$l(D)=\dim_k(\Gamma(X,\mL(D)))$$
\end{defn}
...
\begin{itemize}
\item $l(D)$ is a finite dimensional vector space over $k$. 
\item If $\deg(D)<0$, then we have $$l(D)=0$$
\item If $\deg(D)=0$, then we have $$l(D)=\begin{cases}
0 & \text{ if }D\not\sim0\\
1 & \text{ if }D\sim 0
\end{cases}$$
\end{itemize}
...
\begin{lmm}{}{}\\
Let $k$ be an algebraically closed field. Let $X$ be a smooth irreducible projective curve over $k$. Let $D\in\text{Div}(X)$ be a divisor. Let $p\in X$. Then we have $$0\leq l(D)-l(D-p)\leq 1$$
\begin{proof}\\
...
\begin{prp}{}{}\\
Let $k$ be an algebraically closed field. Let $X$ be a smooth irreducible projective curve over $k$. Let $D\in\text{Div}(X)$ be a divisor. If $\deg(D)\geq 0$, then we have $$l(D)\leq\deg(D)+1$$
\begin{proof}\\
...
Let $k$ be an algebraically closed field. Let $C$ be a smooth irreducible projective curve over $\C$. Let $D\in\text{Div}(C)$ be a divisor on $C$. Then we have $$l(D)-l(K_C-D)=\deg(D)+1-p_g(C)$$
\end{thm}
...
\pagebreak
\section{Morphisms of Curves}
\subsection{The Degree of a Morphism}
\begin{prp}{}{}\\
Let $k$ be an algebraically closed field. Let $X,Y$ be smooth irreducible projective curves over $k$. Let $\phi:X\to Y$ be a regular map. Then exactly one of the following holds. 
\begin{itemize}
...
\begin{defn}{Degree of a Morphism of Curves}{}\\
Let $k$ be an algebraically closed field. Let $X,Y$ be smooth projective irreducible curves over $k$. Let $\phi:X\to Y$ be a non-constant morphism. Define the degree of $\phi$ to be $$\deg(\phi)=[k(X):f^\ast k(Y)]$$
\end{defn}
...
\subsection{Base Point Free and Very Ample Divisors}
Let $C$ be a smooth projective irreducible curve over $\C$. Let $D$ be a divisor on $C$. Recall the associated rational map $F_D:C\to\Prj(\Gamma(X,\mL(D))^\ast)$ given by $$p\mapsto\left(\substack{\phi_p:\mL(D)\to\C\\f\mapsto(f\cdot\pi^{n_p})(p)}\right)$$ where $\pi$ is a uniformizer of $\mO_{C,p}$ and $n_p$ is the coefficient of $p$ in $D$. It is a genuine morphism if and only if $D$ is base point free. 

\begin{prp}{}{}\\
Let $C$ be a smooth projective irreducible curve over $\C$. Let $D\in\text{Div}(C)$ be a divisor. Then $D$ is basepoint free if and only if $$l(D-p)=l(D)-1$$ for all $p\in C$. 
\begin{proof}\\
...
Also recall that the associated map $F_D:C\to\Prj(\Gamma(X,\mL(D))^\ast)$ is a closed embedding if and only if $\mL(D)$ is very ample. 

\begin{prp}{}{}\\
Let $C$ be a smooth projective irreducible curve over $\C$. Let $D\in\text{Div}(C)$. Then $D$ is very ample if and only if for all $p,q\in C$, we have $$l(D-p-q)=l(D)-2$$
\end{prp}

\begin{crl}{}{}\\
Let $C$ be a smooth projective irreducible curve over $\C$. Let $D\in\text{Div}(C)$. If $\deg(D)\geq 2g+1$ then $D$ is very ample. 
\end{crl}
...
\begin{defn}{}{}\\
Let $k$ be an algebraically closed field. Let $C$ be a smooth irreducible projective curve over $k$. We say that $C$ is elliptic if $p_g(C)=1$. 
\end{defn}

\begin{prp}{}{}\\
Let $k$ be an algebraically closed field. Let $C$ be a smooth irreducible projective curve over $k$. Then $C$ is an elliptic curve if and only if $CV$ admits an emebdding $C\to\Prj_k^2$ of degree $3$. 
\end{prp}

\begin{prp}{}{}\\
Let $k$ be an algebraically closed field. Let $C$ be a smooth irreducible projective curve over $k$. Then $C$ is an elliptic curve if and only if it is the isomorphic to the projective closure of $\V(y^2-(x^3+ax+b))$ for some $a,b\in k$ such that $4a^3+27b^2\neq 0$. 
\end{prp}

\begin{prp}{}{}\\
Let $k$ be an algebraically closed field. Let $C$ be an elliptic curve over $k$. Then there is a bijection of sets $$C(k)\cong\text{Pic}^0(C)$$
\end{prp}
