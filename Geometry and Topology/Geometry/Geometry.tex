...
\subsection{Lorentzian Linear Algebra}
\begin{defn}{Lorentz Inner Product}{}\\
Define the Lorentz inner product on $\R^{n+1}$ to be the function $\langle-,-\rangle_L:\R^{n+1}\times\R^{n+1}\to\R$ given by $$\langle(x_1,\dots,x_{n+1}),(y_1,\dots,y_{n+1})\rangle_L=-x_1y_1+\sum_{i=2}^{n+1}x_iy_i$$
\end{defn}

This is the associated symmetric bilinear form of a quadratic form with signature $(n,1)$. 
...
By Sylvester's law of inertia, the matrix of any quadratic form with signature $(n,1)$ is congruent to the same matrix called the Lorentz identity matrix. 

\begin{defn}{Lorentz Identity Matrix}{}\\
Define the Lorentz identity matrix to be $$J_{n+1}=\begin{pmatrix}
-1 & 0\\
0 & I_n
...
This is a basis that is guaranteed by Sylvester's law of inertia. 
...
So Lorentz transformations are just linear maps of $\R^{n+1}$ preserving the quadratic form with signature $(n,1)$. 

\begin{prp}{}{}\\
Let $T:\R^{n+1}\to\R^{n+1}$ be a linear map. Then the following are equivalent. 
\begin{itemize}
\item $T$ is a Lorentz transformation. 
\item $[T]_E^E\in O(n,1)$, where $E$ is the standard basis of $\R^{n+1}$. 
...
\begin{prp}{}{}\\
Let $\F$ be a field. There is a one-to-one correspondence $$\F\Prj^n\;\;\overset{1:1}{\longleftrightarrow}\{V\subseteq\F^{n+1}\;|\;V\text{ is a }1\text{-dimensional subspace of }\F^{n+1}\}$$
\end{prp}
...
\begin{defn}{Group of Affine Isomorphisms}{}\\
Let $(A,V,+)$ be an affine space. Define the group of affine isomorphisms to be the set $$\text{Aff}(A)=\{f:A\to A\;|\;f\text{ is an affine isomorphism }\}$$ together with composition functions. 
\end{defn}

\begin{prp}{}{}\\
Let $(A,V,+)$ be an affine space. Then there is an isomorphism $$\text{Aff}(A)\cong\text{GL}(V)\ltimes\R^n$$
\end{prp}

\subsection{Affine Span of a Subset}
\begin{defn}{Affine Span}{}\\
Let $S\subseteq\R^n$ be a subset. Define the affine span of $S$ to be $$\text{AffSpan}(S)=\left\{\sum_{i=1}^k\lambda_ix_i\;\bigg{|}\;x_i\in S,\lambda_i\in\R,\sum_{i=1}^k\lambda_i=1,k\geq 1\right\}$$
\end{defn}
