...
\begin{lmm}{}{}\\
Let $(X,d)$ be a metric space. Then $$\text{Isom}(X,d)$$ is a group. 
\begin{proof}\\
...
Let $x,y\in \R^n$. The Euclidean Metric is defined to be $$d(x,y)=\|x-y\|$$
\end{defn}

Clearly the Euclidean metric indeed defines a metric on $\R^n$. Any subspaces of $\R^n$ inherits the subspace metric of $\R^n$. In particular, $I=[0,1]$ is given a metric. 

\begin{defn}{Geodesics}{} Let $(X,d)$ be a metric space. A geodesic on $X$ is a distance preserving function $\gamma:I\subseteq\R\to X$. 
\end{defn}
...
\subsection{The Four Centers of a Triangle}
...
\begin{prp}{}{} Let $\triangle PQR$ be a spherical triangle of $S^n$. Then $\angle QOR=d(Q,R)$ and vice versa for the other angles. 
...
\begin{prp}{}{} Let $PQR$ be a spherical triangle of $S^2$. Suppose that $\alpha=\angle_SQPR$, $\beta=\angle_SPRQ$ and $\gamma=\angle_S PQR$. Let $a=\angle QOR=d_{S^2}(Q,R)$. Then $$\cos(\alpha)=\cos(\beta)\cos(\gamma)+\sin(\beta)\sin(\gamma)\cos(a)$$
\begin{proof}
...
\begin{prp}{Triangle Inequality}{} Let $PQR$ be a spherical triangle of $S^n$ whose sides are shorter arcs given by $\alpha,\beta,\gamma\leq\pi$. Then $$\alpha\leq\beta+\gamma$$ with equality if and only if $PQR$ is collinear. 
\begin{proof}
...
\begin{prp}{}{} The spherical metric is indeed a metric on $S^n$. And thus $S^n$ is a metric space. 
\begin{proof}
\end{proof}
\end{prp}
...
\begin{prp}{}{} We have that $$\Prj^n\cong\R^n\cup\R^{n-1}\cup\dots\cup\R\cup\{\infty\}$$ 
\begin{proof}
