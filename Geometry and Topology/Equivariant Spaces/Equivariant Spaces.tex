...
\begin{eg}{}{}\\
Let $n\in\N^+$. Then the action of $\text{O}(n+1)$ on $S^n$ induces a homeomorphism $$\frac{\text{O}(n+1)}{\text{O}(n)}\cong S^n$$ where we consider $\text{O}(n)$ as a subgroup of $\text{O}(n+1)$ by the inclusion $A\mapsto\begin{pmatrix}
A & 0\\
0 & 0
\end{pmatrix}$. 
\begin{proof}\\
Clearly $S^n$ is a homogeneous $\text{O}(n+1)$-space. By choosing $x_0=(0,\dots,0,1)$, we note that $\text{Stab}_{\text{O}(n+1)}((0,\dots,0,1))=\text{O}(n)$. So it follows from the above proposition that we have a homeomorphism. 
\end{proof}
\end{eg}
