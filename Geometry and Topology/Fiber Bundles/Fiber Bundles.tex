...
\item A function of sets $g_{ij}:U_i\cap U_j\to\text{Aut}(F)$ for $i,j\in I$. 
\end{itemize}
...
Let $X$ be a space. Let $G$ be a topological group. Then the coordinate transformations uniquely determines and is determined by a fiber bundle in the following sense: There is a one to one bijection $$\left\{\substack{G\text{-bundles over }B\\\text{with fiber }F}\right\}\;\;\;\;\overset{1:1}{\longleftrightarrow}\;\;\;\;\left\{\substack{\text{An open cover }\{U_i\;|\;i\in I\}\text{ of }B\\\text{Maps }\{g_{ij}:U_i\cap U_j\to G\;|\;i,j\in I\}\text{ satisfying the cocyle condition}\\\text{A space }F\text{ such that }G\text{ acts continuously on}}\right\}$$ given as follows. For a collection of maps $\{g_{ij}:U_i\cap U_j\to G\;|\;i,j\in I\}$, consider the space $$E=\frac{\bigcup_{i\in I}U_i\times F}{\sim}$$ where $(x,k)\sim(x,g_{ij}(x)\cdot k)$ for $x\in U_i\cap U_j$. Then $E$ defines a fiber bundle with the projection map to $B$. 
\end{thm}

\begin{defn}{Associated Fiber Bundle}{}\\
Let $G$ be a topological group. Let $p:E\to B$ be a fiber bundle with structure group $G$. Let $F'$ be a space. Let $\{(U_i,\varphi_i)\;|\;i\in I\}$ be the local trivialization charts of $p$. Let $g_{ij}$ for $i,j\in I$ be the maps governing the transition functions of $p$. Define the associated fiber bundle of $p$ with fiber $F$ to be the fiber bundle constructed with the following data applied to the fiber bundle construction theorem. 
\begin{itemize}
\item Take the open cover as $\{U_i\;|\;i\in I\}$. 
\item Take the collection of maps as $\{g_{ij}\;|\;i\in I\}$. 
\item Take the space as $F'$. 
\end{itemize}
\end{defn}
...
\subsection{}
\begin{defn}{The Homotopy Quotient}{}\\
